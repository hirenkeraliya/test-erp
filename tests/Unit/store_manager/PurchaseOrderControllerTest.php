...
test('It calls the store method of PurchaseOrderQueries class and returns a proper response', function (): void {
    ...
    $purchaseOrderData = new PurchaseOrderData(1, 1, $locationId, null, null, null, null, 1, []);
    ...
    $this->assertStringContainsString('store-manager/purchase-orders', $response->getTargetUrl());
});
...
test(
    ...
    function (): void {
        ...
        $purchaseOrderQueries = $this->mock(PurchaseOrderQueries::class, function ($mock) use (
            ...
            $mock->shouldReceive('getByIdLocationAndCompanyIdWithRelation')
                ->once()
                ->andReturn($purchaseOrder);
            $mock->shouldNotReceive('updateStatus');
        });
