...
test(
    'It calls the listQueryForWarehouseManager method of the goods received note queries class and returns proper response',
    function (): void {
        $locationId = 1;
...
test('It calls the store method and returns a proper response', function (): void {
    ...
    $request->setUserResolver(fn (): WarehouseManager => $user);
    ...
    $this->assertStringContainsString('warehouse-manager/goods-received-notes', $redirectResponse->getTargetUrl());
...
test(
    'It calls the getGoodeReceiveNotesExportForWarehouseManager method and returns a proper response',
    function (): void {
        $companyId = 1;
        $locationId = 1;
        ...
        $goodsReceivedNoteQueries = $this->mock(GoodsReceivedNoteQueries::class, function ($mock) use (
            ...
            $mock->shouldReceive('getGoodeReceiveNotesExportForWarehouseManager')
            ->once()
        ...
        $response = $goodsReceivedNoteController->exportGoodReceivedNote(
            'filename.csv',
