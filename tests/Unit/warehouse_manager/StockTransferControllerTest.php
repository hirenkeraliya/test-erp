...
test(
    'It calls the warehouseManagerListQuery method of the StockTransferQueries class and returns proper response',
    function (): void {
        $locationId = 1;
        $companyId = 1;

        stockTransferSessionIdWarehouse($locationId, $companyId);
        ...
        [$warehouseManager, $request] = setRequestUserForWarehouseManager($requestParameter);
        ...
        $stockTransferQueries = $this->mock(StockTransferQueries::class, function ($mock): void {
            ...
            $mock->shouldReceive('warehouseManagerTransferOrderStatusCount')
                ->times(4)
...
test(
    ...
    function (): void {
        [$request, $product, $stockTransfer, $stockTransferItem, $batch, $stockTransferData, $companyId] = seedWarehouseTransferOrderTypeRecords();
        ...
        $this->mock(StockTransferCheckRequestService::class, function ($mock) use (
            $stockTransferData,
            $location
        ): void {
            $mock->shouldReceive('checkTransferType')
                ->once()
                ->with($stockTransferData, $location->id);
        ...
        $this->assertStringContainsString('warehouse-manager/stock-transfers', $response->getTargetUrl());
    }
);
...
test(
    ...
    function (): void {
        ...
        [$warehouseManager, $request] = setRequestUserForWarehouseManager();
...
test(
    ...
    function (): void {
        ...
        [$user, $request] = setRequestUserForWarehouseManager();
...
test('It calls the updateStatus method while change to open status and returns a proper response', function (): void {
    ...
    stockTransferSessionIdWarehouse($locationId, $companyId);
    [$warehouseOne, $warehouseTwo, $warehouseManagerOne] = seedWarehouseAndWarehouseManagers($companyId);

    $stockTransfer = StockTransfer::factory()->make([
        'company_id' => $companyId,
        'source_location_id' => $warehouseOne->id,
        'destination_location_id' => $warehouseTwo->id,
        'requested_by_id' => 1,
        'stock_transfer_reason_id' => null,
        'status' => StatusTypes::DRAFT->value,
    ]);

    $stockTransfer->sourceLocation = $warehouseOne;
    $stockTransfer->destinationLocation = $warehouseTwo;
...
test(
    ...
    function ($status): void {
        ...
        $stockTransferUpdateStatusData = new StockTransferUpdateStatusData(StatusTypes::CANCELLED->value, 'remarks');
...
)->with([[StatusTypes::DRAFT->value], [StatusTypes::OPEN->value]]);

test(
    'It calls the updateStatus method while change to rejected status and returns a proper response',
    function ($sourceWarehouseId, $destinationWarehouseId, $transferType): void {
        $locationId = 1;
        $companyId = 1;
...
test(
    ...
    function (): void {
        ...
        stockTransferSessionIdWarehouse(1, $companyId);
...
test('It calls the updateStatus method while change to close status and returns a proper response', function (): void {
    ...
    stockTransferSessionIdWarehouse($warehouseTwo->id, $companyId);
...
test(
    ...
    function (): void {
        ...
        stockTransferSessionIdWarehouse(10, $companyId);
        ...
        [$warehouseManager,
            $request] = setRequestUserForWarehouseManager();
...
test(
    ...
    function (): void {
        ...
        stockTransferSessionIdWarehouse(200, $companyId);
...
test(
    ...
    function (): void {
        stockTransferSessionIdWarehouse(1, 1);
...
it(
    ...
    function (): void {
        ...
        [$warehouseManager,
            $request] = setRequestUserForWarehouseManager($additionalItems);
...
function stockTransferSessionIdWarehouse(int $locationId, int $companyId): void
{
    setWarehouseManagerWarehouseIdInSession($locationId);
    setWarehouseManagerWarehouseCompanyIdInSession($companyId);
}

function seedWarehouseAndWarehouseManagers(int $companyId): array
{
    $warehouseOne = Location::factory()->make([
        'id' => 1,
        'company_id' => $companyId,
        'type_id' => LocationTypes::WAREHOUSE->value,
    ]);

    $warehouseManagerOne = WarehouseManager::factory()->make([
        'id' => 1,
        'employee_id' => 2,
    ]);

    $warehouseManagerTwo = WarehouseManager::factory()->make([
        'id' => 2,
        'employee_id' => $companyId,
    ]);

    $warehouseTwo = Location::factory()->make([
        'id' => 2,
        'company_id' => $companyId,
        'type_id' => LocationTypes::WAREHOUSE->value,
    ]);

    $warehouseOne->warehouseManagers = collect([$warehouseManagerOne]);
    $warehouseTwo->warehouseManagers = collect([$warehouseManagerTwo]);

    return [$warehouseOne, $warehouseTwo, $warehouseManagerOne, $warehouseManagerTwo];
}

function seedWarehouseTransferOrderTypeRecords(): array
{
    $locationId = 1;
    $companyId = 1;
...
test('It calls the exportStockTransfers method and returns a proper response', function (): void {
    ...
    $stockTransferQueries = $this->mock(StockTransferQueries::class, function ($mock): void {
        $mock->shouldReceive('getWarehouseManagerStockTransfersExport')
            ->once()
...
test('the fetchAggregateAverageDays method  call and return average days', function (): void {
    ...
    [$warehouseManager, $request] = setRequestUserForWarehouseManager($requestData);
    $companyId = 1;
    setWarehouseManagerWarehouseCompanyIdInSession($companyId);
