...
test('It calls the addNew method of the StockTakeProductQueries class and returns proper response', function (): void {
    ...
    $importRecord = ImportRecord::factory()->make([
        ...
        'created_by_type' => ModelMapping::getCaseName($warehouseManager::class),
...
test(
    'addStockTake method throw exception if pending to submit stock take by warehouse manager id & warehouse id',
    function (): void {
        setWarehouseManagerWarehouseIdInSession();
        setWarehouseManagerWarehouseCompanyIdInSession();
        ...
        StockTake::factory()->make([
            ...
            'location_id' => 1,
            'submitted_by_id' => 1,
            'submitted_by_type' => ModelMapping::WAREHOUSE_MANAGER->name,
    ...
    HttpException::class,
    'A stock take is already pending for the selected location. Please complete it first.'
);
...
test('It calls the submit method of the StockTakeQueries class and returns proper response', function (): void {
    ...
    $this->assertStringContainsString('warehouse-manager/stock-takes', $redirectResponse->getTargetUrl());
...
test(
    ...
    function (): void {
        ...
        $response = $stockTakeController->bulkUpdateStocks($request, $bulkData, $stockTake->id);
