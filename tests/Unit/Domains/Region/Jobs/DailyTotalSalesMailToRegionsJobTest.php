...
test(
    'DailyTotalSalesMailToRegionsJob job calls respective methods and send mail send successfully',
    function (): void {
        ...
        $this->mock(RegionQueries::class, function ($mock) use ($region): void {
            $mock->shouldReceive('getRegionsIdColumn')
                ->once()
                ->andReturn(new Collection([$region]));
        ...
        DailyTotalSalesMailToRegionsJob::dispatch()->onQueue(config('horizon.default_queue_name'));
