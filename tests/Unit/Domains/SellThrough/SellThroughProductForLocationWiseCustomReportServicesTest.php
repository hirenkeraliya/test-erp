...
test(
    'it calls method of the ProductQueries class as expected',
    function (): void {
        $companyId = 1;
        ...
        $this->mock(ProductQueries::class, function ($mock): void {
            $mock->shouldReceive('accumulatedSaleThroughReturnsDataByProductUpcForCustomReportForStoreWise')
                ->once()
                ->andReturn(collect([]));
            $mock->shouldReceive('accumulatedSaleThroughSalesDataByProductUpcForCustomReportForStoreWise')
                ->once()
                ->andReturn(collect([]));
            $mock->shouldReceive('accumulatedSaleThroughInventoryDataByProductUpcForCustomReportForStoreWise')
                ->once()
                ->andReturn(collect([]));
        });
        ...
        $sellThroughProductUpcServices = new SellThroughProductForLocationWiseCustomReportServices();
