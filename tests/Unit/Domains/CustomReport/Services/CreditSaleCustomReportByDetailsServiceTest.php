...
use App\Domains\Sale\Services\CreditSaleCustomReportByDetailsService;
...
it('export function exports sales data by details and item to expected format', function (): void {
    $filterData = [
        'location_ids' => [1],
        'date_range' => [now(), now()],
        'report_type' => CreditReportTypes::BY_DETAILS->value,
        'counter_ids' => [],
        'cashier_ids' => [],
    ];
    ...
    $sale->creditAuthorizer = $storeManager;
    ...
    $this->mock(SaleQueries::class, function ($mock) use ($sale): void {
        $mock->shouldReceive('getCreditSalesWithItemsData')
            ->once()
            ->andReturn(collect([$sale]));
    });

    $creditSaleCustomReportByDetailsService = new CreditSaleCustomReportByDetailsService();
    $result = $creditSaleCustomReportByDetailsService->export($filterData, $company->id, 'demo.csv');
