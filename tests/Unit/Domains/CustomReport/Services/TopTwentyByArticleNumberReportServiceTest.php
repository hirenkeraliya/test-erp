...
use App\Domains\Sale\Services\TopTwentyByArticleNumberReportService;
...
it('exports the top twenty article number report for a given company as an Excel file', function (): void {
    ...
    $filterData = [
        ...
        'counter_ids' => [],
        'filter_by' => null,
        'report_type' => [TopTwentyReportTypes::BY_MASTER_PRODUCT->value],
    ...
    $this->mock(TopTwentyAggregateDataQueries::class, function ($mock) use ($saleItem): void {
        $mock->shouldReceive('getByStoreForTopProductExport')
            ->once()
            ->andReturn(collect([$saleItem]));
    });

    $topTwentyByArticleNumberReportService = resolve(TopTwentyByArticleNumberReportService::class);
    $result = $topTwentyByArticleNumberReportService->export($company->id, $filterData, 'demo.xlsx');

    expect($result)->toBeInstanceOf(BinaryFileResponse::class);
});

it('prints the top twenty article number report for a given company and returns a string', function (): void {
    $company = Company::factory()->make([
        'id' => 1,
        'default_country_id' => 1,
    ]);
    ...
    $topTwentyByArticleNumberReportService = resolve(TopTwentyByArticleNumberReportService::class);
    $response = $topTwentyByArticleNumberReportService->print($company->id, $filterData);
