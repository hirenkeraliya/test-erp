...
use App\Domains\Sale\Services\TopTwentyByColorReportService;
...
it('exports the top twenty color report for a given company as an Excel file', function (): void {
    ...
    $filterData = [
        ...
        'cashier_ids' => [],
        'counter_ids' => [],
        'report_type' => [TopTwentyReportTypes::BY_COLORS->value],
    ...
    $this->mock(ColorQueries::class, function ($mock) use ($color): void {
        $mock->shouldReceive('getByCompanyId')
            ->once()
            ->andReturn(new Collection([$color]));
    });
    ...
    $topTwentyByColorReportService = resolve(TopTwentyByColorReportService::class);
    $result = $topTwentyByColorReportService->export($company->id, $filterData, 'demo.xlsx');

    expect($result)->toBeInstanceOf(BinaryFileResponse::class);
});

it('prints the top twenty color report for a given company and returns a string', function (): void {
    $company = Company::factory()->make([
        'id' => 1,
        'default_country_id' => 1,
    ]);
    ...
    $topTwentyByColorReportService = resolve(TopTwentyByColorReportService::class);
    $response = $topTwentyByColorReportService->print($company->id, $filterData);
