...
beforeEach(function (): void {
    $this->bundleBuyPromotionService = new BundleBuyPromotionService();
...
test(
    ...
    function (): void {
        ...
        $this->bundleBuyPromotionService->checkForApplicability(
...
)->throws(HttpException::class, 'group id is required for bundle buy promotion but not passed.');
...
test(
    ...
    function (): void {
        ...
        $mock = $this->createPartialMock(
            BundleBuyPromotionService::class,
...
test(
    'checkForApplicability method sets the saleMismatches when Some of the discountable products are not matched with our records for the bundle buy promotion',
    function (): void {
        $this->checkSaleDetailsService->saleMismatches = collect([]);
    ...
    HttpException::class,
    'Some of the specified buy products do not match with our records for the bundle buy promotion.'
);

test(
    'checkForApplicability method sets the saleMismatches when Specified promotion discountable quantities are more than requested quantities',
    function (): void {
        $this->checkSaleDetailsService->saleMismatches = collect([]);
        $this->saleDetails['items'][0]['group_id'] = 1;
        $this->saleDetails['items'][0]['item_discount_amount'] = 1;
...
)->throws(HttpException::class, 'Only 5 units are eligible for the bundle buy promotion. But requested units are 10.');
...
test(
    ...
    function (): void {
        ...
        $response = $this->bundleBuyPromotionService->getPromotionTierValue(
...
test('getTotalApplicableQuantities method returns the applicable quantities as expected', function (): void {
    ...
    $response = $this->bundleBuyPromotionService->getTotalApplicableQuantities(11, $this->promotion);
    $this->assertEquals(10, $response);
});

test(
    'getTotalDiscountAmount method returns the Discount as expected',
    function ($applicableItemTotal, $totalBuyQuantity, $discount): void {
        $this->saleDetails['items'][0]['item_discount_amount'] = 1;
        ...
        $response = $this->bundleBuyPromotionService->getTotalDiscountAmount(
            $this->promotion,
            $applicableItemTotal,
            $totalBuyQuantity,
            0,
...
)->with([[50, 5, 10], [50, 4, 0], [50, 6, 10], [100, 10, 20]]);

test(
    'getTotalGetValue method returns the Discount as expected',
    function ($applicableItemTotal, $totalBuyQuantity, $discount): void {
        $this->saleDetails['items'][0]['item_discount_amount'] = 1;
        ...
        $response = $this->bundleBuyPromotionService->getTotalGetValue(
            $this->promotion,
            $applicableItemTotal,
            $totalBuyQuantity,
            0,
...
)->with([[50, 5, 40], [50, 4, 50], [50, 6, 40], [100, 10, 80]]);
...
test('calculateItemDiscountAmount method return last item discount', function (): void {
    ...
    $this->promotion->promotionTiers = collect([$promotionTier]);
    $this->saleDetails['items'][1]['item_discount_amount'] = 10;
    $response = $this->bundleBuyPromotionService->calculateItemDiscountAmount(
...
test(
    'calculateItemDiscountAmount method return last item discount when pass discount_item_sequence',
    function (): void {
        $this->saleDetails['items'][0]['item_discount_amount'] = 2;
        $this->saleDetails['items'][0]['discount_item_sequence'] = 1;
...
test('isDiscountItemSequence method returns boolean as expected', function (): void {
    $cartItem = [];
    $response = $this->bundleBuyPromotionService->isDiscountItemSequence($cartItem);
...
test('isDiscountReturn method returns boolean as expected', function (): void {
    $cartItem['id'] = 1;
    $item['id'] = 2;
    $response = $this->bundleBuyPromotionService->isDiscountReturn($cartItem, $item);
    $this->assertFalse($response);
...
test('matchItemByDiscountItemSequence method returns boolean as expected', function (): void {
    $cartItem['discount_item_sequence'] = 1;
    $item['discount_item_sequence'] = 2;
    $response = $this->bundleBuyPromotionService->matchItemByDiscountItemSequence($cartItem, $item);
    $this->assertFalse($response);
...
)->throws(HttpException::class, 'discount item sequence is required for bundle buy promotion but not passed.');
...
test(
    'checkForApplicability method sets the saleMismatches when the discount_item_sequence is 0',
    function (): void {
        $this->checkSaleDetailsService->saleMismatches = collect([]);
        $this->saleDetails['items'][0]['group_id'] = 1;
        $this->saleDetails['items'][0]['discount_item_sequence'] = 0;
        $this->saleDetails['items'][0]['item_discount_amount'] = 1;
        $this->saleDetails['items'][0]['promotion_id'] = 1;
        $this->saleData = new SaleData(...$this->saleDetails);
...
test('getItemDiscountAmount method return 0 when not set item_discount_amount in cart', function (): void {
    ...
    $response = $this->bundleBuyPromotionService->getItemDiscountAmount($cartItem);
