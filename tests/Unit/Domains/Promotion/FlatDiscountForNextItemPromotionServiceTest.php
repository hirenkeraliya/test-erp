...
beforeEach(function (): void {
    $this->flatDiscountForNextItemPromotionService = new FlatDiscountForNextItemPromotionService();
    ...
    $this->promotion = Promotion::factory()->make([
        'id' => 1,
        'company_id' => 1,
        'name' => 'Flat Discount For Next Item Promotion',
...
test(
    ...
    function (): void {
        ...
        $this->flatDiscountForNextItemPromotionService->checkForApplicability(
...
)->throws(HttpException::class, 'group id is required for Flat Discount For Next Item promotions but not passed.');
...
test(
    ...
    function (): void {
        ...
        $mock = $this->createPartialMock(
            FlatDiscountForNextItemPromotionService::class,
...
test(
    ...
    HttpException::class,
    'discount item sequence is required for Flat Discount For Next Item promotions but not passed.'
);
...
test(
    ...
    HttpException::class,
    'Some of the specified discountable products do not match with our records for the Flat Discount For Next Item promotion.'
);
...
test('getPromotionTier method returns response as expected', function (): void {
    ...
    $response = $this->flatDiscountForNextItemPromotionService->getPromotionTier(30, $this->promotion);
...
test(
    ...
    function (float $itemTotal, float $discount): void {
        ...
        $mock = $this->createPartialMock(FlatDiscountForNextItemPromotionService::class, ['getPromotionTier']);
...
test('getItemDiscountAmount method return 0 when not set item_discount_amount in cart', function (): void {
    ...
    $response = $this->flatDiscountForNextItemPromotionService->getItemDiscountAmount($cartItem);
...
test(
    'checkForApplicability method sets the saleMismatches when the item discount amount is negative',
    function (): void {
        $this->checkSaleDetailsService->saleMismatches = collect([]);
        $this->saleDetails['items'][0]['group_id'] = 1;
        $this->saleDetails['items'][0]['discount_item_sequence'] = 1;
