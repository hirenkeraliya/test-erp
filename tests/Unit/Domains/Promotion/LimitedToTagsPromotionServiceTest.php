...
beforeEach(function (): void {
    $this->limitedToTagsPromotionService = new LimitedToTagsPromotionService();
    ...
    $this->promotion = Promotion::factory()->make([
        'id' => 1,
        'company_id' => 1,
        'name' => 'Item wise limited to categories',
...
test(
    'checkForApplicability method sets the saleMismatches when none of the product tags are in the promotion tags',
    function (): void {
        ...
        $this->product->tags = collect($this->tag);

        $this->promotion->tags = collect(new Tag());
        ...
        $this->limitedToTagsPromotionService->checkForApplicability(
...
)->throws(HttpException::class, 'Specified promotion is not applicable on the given product tag ABC');
...
test(
    ...
    function (): void {
        ...
        $this->promotion->tags = collect([$this->tag]);
...
test(
    ...
    function (): void {
        ...
        $mock = $this->createPartialMock(
            LimitedToTagsPromotionService::class,
...
test(
    ...
    function ($itemTotal, $percentage): void {
        ...
        $response = $this->limitedToTagsPromotionService
...
test('getItemDiscountAmount method return 0 when not set item_discount_amount in cart', function (): void {
    ...
    $response = $this->limitedToTagsPromotionService->getItemDiscountAmount($cartItem);
