...
beforeEach(function (): void {
    $this->BuyTwoGetRMFiftyOffOnOthersPromotionService = new BuyTwoGetRMFiftyOffOnOthersPromotionService();
...
test(
    ...
    function (): void {
        ...
        $this->BuyTwoGetRMFiftyOffOnOthersPromotionService->checkForApplicability(
...
)->throws(HttpException::class, 'group id is required for Buy 2 get RM50 off on others promotions but not passed.');
...
test(
    ...
    function (): void {
        ...
        $mock = $this->createPartialMock(
            BuyTwoGetRMFiftyOffOnOthersPromotionService::class,
...
test(
    'checkForApplicability method sets the saleMismatches when Some of the specified discounted products do not match with our records for the Buy 2 get RM50 off on others promotion',
    function (): void {
        $this->checkSaleDetailsService->saleMismatches = collect([]);
    ...
    HttpException::class,
    'Some of the specified discounted products do not match with our records for the Buy 2 get RM50 off on others promotion.'
);

test(
    'checkForApplicability method sets the saleMismatches when Some of the buy products are not matched with our records for the Buy 2 get RM50 off on others promotion',
    function (): void {
        $this->checkSaleDetailsService->saleMismatches = collect([]);
        $this->saleDetails['items'][0]['group_id'] = 1;
        $this->saleDetails['items'][0]['promotion_id'] = 1;
    ...
    HttpException::class,
    'Some of the specified buy products do not match with our records for the Buy 2 get RM50 off on others promotion.'
);

test(
    'checkForApplicability method sets the saleMismatches when Requested discount quantities for the Buy 2 get RM50 off on others promotion is more than 1',
    function (): void {
        $this->checkSaleDetailsService->saleMismatches = collect([]);
        $this->saleDetails['items'][0]['group_id'] = 1;
        $this->saleDetails['items'][0]['promotion_id'] = 1;
    ...
    HttpException::class,
    'Only 2 units can be given for free for this Buy 2 get RM50 off on others promotion. But requested discounted quantities are 10.'
);
...
test(
    ...
    function ($totalBuyQuantity, $getValue): void {
        ...
        $response = $this->BuyTwoGetRMFiftyOffOnOthersPromotionService->getTotalDiscountableQuantities(
            $totalBuyQuantity,
            $this->promotion
        );
...
test('getPromotionTierValue method returns response as expected', function (): void {
    ...
    $response = $this->BuyTwoGetRMFiftyOffOnOthersPromotionService->getPromotionTierValue(20, $this->promotion);
...
test('getItemDiscountAmount method return 0 when not set item_discount_amount in cart', function (): void {
    ...
    $response = $this->BuyTwoGetRMFiftyOffOnOthersPromotionService->getItemDiscountAmount($cartItem);
