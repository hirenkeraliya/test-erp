...
test(
    'first_name, mobile_number, stores, username , password, passcode are required for import record',
    function (): void {
        ...
        $this->mock(StoreManagerQueries::class, function ($mock): void {
            $mock->shouldReceive('doExistsByEmployeeId')
                ->once();
            $mock->shouldReceive('doExistsByStoreManagerUsername')
              ->once();
        });

        $importStoreManager = new ImportStoreManager();
        $redirectResponse = $importStoreManager->validate($storeManagerData, $importRecord);
        $this->assertEquals(8, is_countable($redirectResponse) ? count($redirectResponse) : 0);
    }
);

test('It calls addNew method to store store manager details', function (): void {
    $company = Company::factory()->make([
        'id' => 1,
        'name' => 'Test Company',
    ...
    $importStoreManager = new ImportStoreManager();
    $importStoreManager->save($storeManagerRecord, $importRecord);
});
