...
test('validate method returns blank array', function (): void {
    $companyId = 1;
    $locationBulkUpdateData = getLocationBulkUpdateData();
    ...
    $this->mock(LocationQueries::class, function ($mock) use (
        $locationBulkUpdateData,
        $companyId,
        $locationType
    ): void {
        $mock->shouldReceive('existsByPhoneAndTypeId')
            ->once()
            ->with($locationBulkUpdateData['phone'], $locationType, $companyId)
            ->andReturn(true);
        $mock->shouldReceive('isLocationNameTakenByAnother')
            ->once()
            ->with($locationBulkUpdateData['name'], $locationBulkUpdateData['phone'], $locationType, $companyId)
            ->andReturn(false);
        $mock->shouldReceive('isLocationCodeTakenByAnother')
            ->once()
            ->with($locationBulkUpdateData['code'], $locationBulkUpdateData['phone'], $locationType, $companyId)
            ->andReturn(false);
    });

    $this->mock(CountryQueries::class, function ($mock) use ($locationBulkUpdateData): void {
        $mock->shouldReceive('existsByName')
            ->once()
            ->with($locationBulkUpdateData['country'])
            ->andReturn(true);
    });

    $this->mock(StateQueries::class, function ($mock) use ($locationBulkUpdateData): void {
        $mock->shouldReceive('existsByName')
            ->times()
            ->with($locationBulkUpdateData['state'])
            ->andReturn(true);
    });

    $this->mock(CityQueries::class, function ($mock) use ($locationBulkUpdateData): void {
        $mock->shouldReceive('existsByName')
            ->times()
            ->with($locationBulkUpdateData['city'])
            ->andReturn(true);
    });
    ...
    $importLocationBulkUpdate = new ImportLocationBulkUpdate();
    $redirectResponse = $importLocationBulkUpdate->validate($locationBulkUpdateData, $importRecord);
...
test('name, brand, code and phone are require while import record', function (): void {
    ...
    $importLocationBulkUpdate = new ImportLocationBulkUpdate();
    $redirectResponse = $importLocationBulkUpdate->validate($locationData, $importRecord);
...
test('save method saves the data', function (): void {
    ...
    $importRecord = getImportRecordsForLocationBulkUpdate($companyId);
    ...
    $importLocationBulkUpdate = new ImportLocationBulkUpdate();
    $importLocationBulkUpdate->save($locationBulkUpdateData, $importRecord);
});
...
test(
    ...
    function (): void {
        ...
        $locationBulkUpdateData['price_fall_down_percentage'] = 120;
...
test('validate Import Location Bulk Update Import Columns', function (): void {
    $requiredHeaderColumns = LocationImportColumns::getArrayValues();

    $importLocationBulkUpdate = new ImportLocationBulkUpdate();
    $response = $importLocationBulkUpdate->validateColumns($requiredHeaderColumns, [], 1);
    $this->assertTrue(ColumnValidationIssueTypes::COLUMN_ISSUE->value === $response['type']);
    $this->assertTrue($response['status']);
});

function getLocationBulkUpdateData(): array
{
    return [
        'type' => 'Store',
        'name' => 'location test',
...
function getImportRecordsForLocationBulkUpdate(int $companyId): ImportRecord
{
    return ImportRecord::factory()->make([
        'company_id' => $companyId,
        'type_id' => ImportTypes::LOCATION_BULK_UPDATE->value,
        'created_by_id' => 1,
