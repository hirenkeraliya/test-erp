...
test('It calls addNew method of the director queries class', function (): void {
    ...
    $directorQueries = $this->mock(DirectorQueries::class, function ($mock) use ($directorRecord, $admin): void {
        $mock->shouldReceive('addNew')
            ->once()
            ->with($directorRecord, $admin);
    });
    ...
    $this->assertStringContainsString('admin/directors', $redirectResponse->getTargetUrl());
});
...
test(
    ...
    function (): void {
        ...
        ->has('director', fn (Assert $director): Assert => $director->where('employee_id', 1)->has('locations', 2))
        );
    }
);
...
test('It calls update method of the director queries class', function (): void {
    ...
    $this->assertStringContainsString('admin/director', $redirectResponse->getTargetUrl());
});
...
function seedDirectorRecord(): array
{
    return [
        'employee_id' => 1,
        'passcode' => '123456',
