...
test(
    'getMatchingUpcProductWithStore method calls the getActiveProductsByUpcAndStoreCode method of the inventory class and returns proper response',
    function (): void {
        $importProductData =
            [
                'import_product_upc' => [
        ...
        $request = new Request($importProductData);

        setCompanyIdInSession(1);

        $this->mock(InventoryQueries::class, function ($mock): void {
            $mock->shouldReceive('getActiveProductsByUpcAndStoreCode')
                ->once()
                ->andReturn(collect([]));
        });

        $inventoryController = new InventoryController();
        $response = $inventoryController->getMatchingUpcProductWithStore($request);

        expect($response)
            ->toHaveKeys(['products', 'products_count']);
    }
);
