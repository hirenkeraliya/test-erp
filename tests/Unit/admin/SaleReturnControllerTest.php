...
test(
    'It calls the get paginated sale returns with relations method of the sale return queries class and returns proper response',
    function (): void {
        $companyId = 1;
        ...
        $saleQueries = $this->mock(SaleReturnQueries::class, function ($mock) use (
            ...
            $mock->shouldReceive('getPaginatedSaleReturnsWithRelations')
                ->once()
...
test('It calls the exportSaleReturns method and returns a proper response', function (): void {
    ...
    $saleReturnQueries = $this->mock(SaleReturnQueries::class, function ($mock) use ($requestParameter): void {
        $mock->shouldReceive('getSaleReturnsWithRelationsForExport')
            ->once()
            ->with($requestParameter, 1)
...
test(
    ...
    function (): void {
        ...
        $saleReturnController = new SaleReturnController($saleReturnQueries);
        $response = $saleReturnController->fetchSaleReturnItems(1);
        expect($response)
