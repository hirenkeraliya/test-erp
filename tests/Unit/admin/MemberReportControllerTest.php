...
test(
    'It calls the get paginated members report method returns proper response response and counts',
    function (): void {
        $companyId = 1;
        ...
        $memberQueries = $this->mock(MemberQueries::class, function ($mock) use (
            ...
            $mock->shouldReceive('getPaginatedMemberReport')
                ->once()
...
test('It calls the export members report and returns a proper response', function (): void {
    $companyId = 1;
    setCompanyIdInSession($companyId);
    ...
    $memberQueries = $this->mock(MemberQueries::class, function ($mock): void {
        $mock->shouldReceive('getMembersReportForExport')
            ->once()
...
test('It calls the fetchMembersDetails method and returns a proper response', function (): void {
    $companyId = 1;
    setCompanyIdInSession($companyId);

    $requestParameter = [
        'select_date' => Carbon::now()->format('Y-m-d'),
        'select_location_id' => 1,
    ];
