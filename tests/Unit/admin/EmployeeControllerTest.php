...
test('It calls the addNew method of the employee queries class and returns proper response', function (): void {
    ...
    $this->assertStringContainsString('admin/employees', $redirectResponse->getTargetUrl());
});
...
test('It calls the update method of the employee queries class and returns proper response', function (): void {
    ...
    $employeeQueries = $this->mock(EmployeeQueries::class, function ($mock) use ($employeeData, $admin): void {
        $mock->shouldReceive('update')
            ->once()
            ->with($employeeData, $admin, 1000);
    });
...
test('it calls the adminSetStatus method of employeeQueries class', function (): void {
    ...
    $employeeQueries = $this->mock(EmployeeQueries::class, function ($mock) use ($employee, $admin): void {
        $mock->shouldReceive('adminSetStatus')
            ->once()
            ->with($employee->id, 1, false, $admin);
    });
    ...
    $this->assertStringContainsString('admin/employees', $response->getTargetUrl());
});
...
test('exportExistingEmployees method and returns a binary file response', function (): void {
    $companyId = 1;
    setCompanyIdInSession($companyId);
    ...
    $employeeRecords = Employee::factory()->make([
        'id' => 1,
        'company_id' => $companyId,
        'designation_id' => $designation->id,
    ]);

    $employeeRecords->designation = $designation;

    $employeeQueries = $this->mock(EmployeeQueries::class, function ($mock) use ($companyId, $employeeRecords): void {
        $mock->shouldReceive('getEmployeeForBulkUpdate')
            ->once()
            ->with($companyId)
            ->andReturn(collect([$employeeRecords]));
    });
    $employeeController = new EmployeeController($employeeQueries);
    $response = $employeeController->exportExistingEmployees();
