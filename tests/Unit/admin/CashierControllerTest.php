...
test('It calls addNew method of the cashier queries class', function (): void {
    ...
    $cashierQueries = $this->mock(CashierQueries::class, function ($mock) use ($cashierRecord, $admin): void {
        $mock->shouldReceive('addNew')
            ->once()
            ->with($cashierRecord, $admin);
    });
    ...
    $this->assertStringContainsString('admin/cashiers', $redirectResponse->getTargetUrl());
});
...
test('It calls the exportBulkUpdateCashiers method and returns a proper response', function (): void {
    $companyId = 1;

    setCompanyIdInSession($companyId);

    $cashierQueries = $this->mock(CashierQueries::class, function ($mock) use ($companyId): void {
        $mock->shouldReceive('getCashiersForBulkUpdate')
            ->once()
            ->with($companyId)
            ->andReturn(collect(new Cashier()));
    });
    ...
    $response = $cashierController->exportBulkUpdateCashiers();
...
test(
    ...
    function (): void {
        ...
        $response = $cashierController->getStoreCashiers($locationId);
...
function cashierDataArray(): array
{
    $cashier = Cashier::factory()->make([
        'employee_id' => 1,
        'cashier_group_id' => 1,
