...
test(
    'It calls the get paginated pending layaway sales with relations method of the sale queries class and returns proper response',
    function (): void {
        $companyId = 1;
        ...
        $saleQueries = $this->mock(SaleQueries::class, function ($mock) use ($requestParameter, $companyId): void {
            $mock->shouldReceive('getPaginatedPendingLayawaySalesWithRelations')
            ->once()
...
test('It calls the exportLayawaySales method and returns a proper response', function (): void {
    ...
    $saleQueries = $this->mock(SaleQueries::class, function ($mock) use ($requestParameter): void {
        $mock->shouldReceive('getPendingLayawaySalesWithRelationsForExport')
            ->once()
...
test(
    ...
    function (): void {
        ...
        $saleQueries = $this->mock(SaleQueries::class, function ($mock) use ($companyId, $sale): void {
            $mock->shouldReceive('getLayawaySaleItemsBy')
                ->once()
        ...
        $response = $layawaySaleController->fetchLayawaySaleItemsBySaleId($sale->id);
