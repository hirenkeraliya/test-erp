...
use App\Domains\StockTake\Exports\StockTakeExportForWarehouseManager;
use App\Domains\StockTake\Jobs\StockTakeJob;
use App\Domains\StockTake\Resources\WarehouseStockTakeListResource;
...
class StockTakeController extends Controller
{
    ...
    public function fetchStockTake(Request $request): array
    {
        ...
        return [
            ...
            'data' => WarehouseStockTakeListResource::collection($lengthAwarePaginator->getCollection()),
        ];
    }
    ...
    public function addStockTake(Request $request): RedirectResponse
    {
        ...
        $anyPendingStockTakeByWarehouseManager = $this->stockTakeQueries->anyPendingStockTakeByManager(
            $locationId,
            $companyId
        );

        if ($anyPendingStockTakeByWarehouseManager) {
            abort(412, 'A stock take is already pending for the selected location. Please complete it first.');
        }

        $validatedData['company_id'] = $companyId;
        $validatedData['requested_by_id'] = $warehouseManager->id;
        $validatedData['requested_by_type'] = ModelMapping::WAREHOUSE_MANAGER->name;
        ...
        try {
            ...
            return to_route('warehouse_manager.stock_takes.index')
                ->with(
                    'success',
    ...
    public function stockTakeProducts(int $stockTakeId): Response|RedirectResponse
    {
        ...
        if ($isStockTakePending) {
            throw new RedirectWithErrorException(
                'warehouse_manager.stock_takes.index',
    ...
    public function submitStockTake(Request $request, int $stockTakeId): RedirectResponse
    {
        ...
        try {
            ...
            return redirect(route('warehouse_manager.stock_takes.index'))
    ...
    public function bulkUpdateStocks(
        ...
        try {
            ...
            $importRecord = $importRecordQueries->addNew($importRecordData, $warehouseManager, $companyId, $stockTakes);

            DB::commit();

            ImportRecordsJob::dispatch($importRecord)->onQueue('high');

            return back()->with('success', 'Bulk update for submitted stock performed successfully.');
    ...
    public function exportStockTakes(string $filename, Request $request): BinaryFileResponse
    {
        ...
        return Excel::download(new StockTakeExportForWarehouseManager($stockTakes), $filename);
    }
