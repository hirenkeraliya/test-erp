...
class PurchaseOrderController extends Controller
{
    ...
    public function store(PurchaseOrderData $purchaseOrderData): RedirectResponse
    {
        ...
        try {
            ...
            if ($purchaseOrderData->order_type === OrderTypes::TRANSFER_REQUEST->value) {
                return to_route('warehouse_manager.purchase_orders.index')
    ...
    public function cancel(Request $request, int $purchaseOrderId): RedirectResponse
    {
        ...
        try {
            $purchaseOrderService->purchaseOrderMarkAsCanceled($purchaseOrder, $warehouseManager);

            DB::commit();

            return to_route('warehouse_manager.purchase_orders.index')->with(
                'success',
    ...
    public function approve(Request $request, int $purchaseOrderId): RedirectResponse
    {
        ...
        try {
            $purchaseOrderService->purchaseOrderApprove($purchaseOrder, $warehouseManager);
    ...
    public function reject(Request $request, int $purchaseOrderId): RedirectResponse
    {
        ...
        try {
            $purchaseOrderService->purchaseOrderMarkAsRejected($purchaseOrder, $warehouseManager);
    ...
    public function edit(int $purchaseOrderId): Response
    {
        ...
        $locationQueries = resolve(LocationQueries::class);
        $locations = $locationQueries->getWithBasicColumns($companyId);
        $stores = $locations->where('type_id', LocationTypes::STORE->value);
        $warehouses = $locations->where('type_id', LocationTypes::WAREHOUSE->value);
    ...
    public function exportPurchaseOrders(string $filename, Request $request): BinaryFileResponse
    {
        ...
        return $purchaseOrderService->exportPurchaseOrders($filterData, $companyId, $filename);
    }
