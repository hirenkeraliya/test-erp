...
class CashierController extends Controller
{
     ...
     */
    public function getStoreCashiers(): array
    {
        $cashiers = $this->cashierQueries->getCashierListOfSelectedLocation(
    ...
    public function getSpecificStoresCashiers(Request $request): array
    {
        $locationIds = $request->get('location_ids');

        $cashiers = $this->cashierQueries->getCashiersOfLocations(
            $locationIds,
            session('store_manager_selected_location_company_id')
        );
    ...
    public function fetchCashiers(Request $request): array
    {
        ...
        $lengthAwarePaginator = $this->cashierQueries->listQuery(
            $filterData,
            session('store_manager_selected_location_company_id')
        );
    ...
    {
        [$locations, $employees, $cashierGroups] = $this->fetchCommonRecords(
            session('store_manager_selected_location_company_id')
        );
    ...
    {
        $this->validateSelectedRecordsWithCompany(session('store_manager_selected_location_company_id'), $cashierData);
        ...
        try {
            ...
            return to_route('store_manager.cashiers.index')
                ->with('success', 'Cashier added successfully.');
        } catch (Throwable $throwable) {
            Log::error('Store Manager Add Cashier', [
    ...
    {
        $cashier = $this->cashierQueries->getByIdWithLocations(
            $cashierId,
            session('store_manager_selected_location_company_id')
        );
        [$locations, $employees, $cashierGroups] = $this->fetchCommonRecords(
    ...
    public function update(CashierData $cashierData, int $cashierId): RedirectResponse
    {
        ...
        try {
            ...
            Log::error('Store Manager Update Cashier', [
                'error_message' => $throwable->getMessage(),
                'error_code' => 'Error code: ' . $throwable->getCode(),
                'file' => 'File: ' . $throwable->getFile(),
                'line' => 'Line: ' . $throwable->getLine(),
    ...
    {
        $cashier = $this->cashierQueries->getById($cashierId, session('store_manager_selected_location_company_id'));
    ...
    public function exportCashiers(string $filename, Request $request): BinaryFileResponse
    {
        ...
        $cashiers = $this->cashierQueries->getCashiersExport(
            $filterData,
            session('store_manager_selected_location_company_id')
        );
