...
class StockTransferController extends Controller
{
    ...
    public function fetchStockTransfers(Request $request): array
    {
        ...
        $lengthAwarePaginator = $this->stockTransferQueries->storeManagerListQuery(
            $filterData,
            session('store_manager_selected_location_company_id'),
            session('store_manager_selected_location_id'),
        );
    ...
    public function store(StockTransferData $stockTransferData, Request $request): RedirectResponse
    {
        ...
        try {
            ...
            return to_route('store_manager.stock_transfers.index')
    ...
    public function update(
        ...
        if (
            ...
            throw new RedirectBackWithErrorException('Stock Transfer should be draft to updates the record.');
        }
    ...
    public function editRequestOrder(int $stockTransferId): Response
    {
        ...
        if (
            ...
            throw new RedirectBackWithErrorException(
                'Stock Transfer should be open & request order type to edit the record.'
            );
        }
    ...
    public function updateRequestOrder(
        ...
        if (
            ...
            throw new RedirectBackWithErrorException(
                'Stock Transfer should be open & request order type to updates the record.'
            );
        }
    ...
    public function updateStatus(
        ...
        try {
            ...
            return to_route('store_manager.stock_transfers.index')->with('success', $message);
        } catch (Throwable $throwable) {
            Log::error('Stock Transfer Update Status', [
                'error_message' => 'Error message: ' . $throwable->getMessage(),
                'error_code' => 'Error code: ' . $throwable->getCode(),
                'file' => 'File: ' . $throwable->getFile(),
                'line' => 'Line: ' . $throwable->getLine(),
    ...
    public function deliveryNote(int $stockTransferId): Response
    {
        ...
        if ($stockTransfer->status !== StatusTypes::RECEIVED->value) {
            throw new RedirectBackWithErrorException("Stock Transfer status must be 'Received' to add delivery notes.");
        }
    ...
    public function closeStockTransfer(Request $request, int $stockTransferId): RedirectResponse
    {
        ...
        if ($stockTransfer->getStatus() !== $receivedStatus) {
            throw new RedirectBackWithErrorException("Stock Transfer status must be 'Received' to close it.");
        }

        if ($this->isAuthorizedDestinationLocation($stockTransfer, $locationId)) {
            throw new RedirectBackWithErrorException('Your are not authorized to close the stock transfer.');
        }
        ...
        if ($anyReceivedQuantityPending) {
            throw new RedirectBackWithErrorException(
                'Received quantity is pending for one of the stock transfer items.'
            );
        }
        ...
        try {
            ...
            return to_route('store_manager.stock_transfers.index')->with(
                'success',
                'Stock Transfer Closed Successfully.'
            );
        } catch (Throwable $throwable) {
            Log::error('Store-Manager-Close-Stock-Transfer', [
                'error_message' => 'Error message: ' . $throwable->getMessage(),
                'error_code' => 'Error code: ' . $throwable->getCode(),
                'file' => 'File: ' . $throwable->getFile(),
                'line' => 'Line: ' . $throwable->getLine(),
    ...
    public function setReceivedQuantitySameAsQuantity(int $stockTransferId): RedirectResponse
    {
        ...
        try {
            ...
            Log::error('Store-Manager-Set-Received-Quantities-Same-As-Quantity', [
    ...
    public function updateShippingDetailsAndMarkAsApproved(Request $request, int $stockTransferId): RedirectResponse
    {
        ...
        if ($stockTransfer->transfer_type !== StockTransferTypes::REQUEST_ORDER->value) {
            throw new RedirectBackWithErrorException(
                "This stock transfer cannot be approved as it is not a 'Request Order'."
            );
        }

        if ($stockTransfer->getStatus() !== $openStatus) {
            throw new RedirectBackWithErrorException(
                'The Stock Transfer status must be open in order to change it to approved.'
            );
        }
        ...
        try {
            ...
            Log::error('Store-Manager-Batch-details-Ship', [
                'error_message' => 'Error message: ' . $throwable->getMessage(),
                'error_code' => 'Error code: ' . $throwable->getCode(),
                'file' => 'File: ' . $throwable->getFile(),
                'line' => 'Line: ' . $throwable->getLine(),
    ...
    public function discrepancy(int $stockTransferId): Response
    {
        ...
        if ($stockTransfer->status !== StatusTypes::DISCREPANCY->value) {
            throw new RedirectBackWithErrorException(
                "Stock Transfer status must be 'Discrepancy' to edit the records."
            );
        }
        ...
        if ($stockTransferItems->isEmpty()) {
            throw new RedirectWithErrorException(
                'store_manager.stock_transfers.index',
                'This Stock Transfer does not have any items.'
            );
        }
    ...
    public function discrepancyProof(Request $request, int $stockTransferId, int $stockTransferItemId): RedirectResponse
    {
        ...
        if ($stockTransfer->getStatus() !== StatusTypes::RECEIVED->value) {
            throw new RedirectBackWithErrorException(
                "The status of the stock transfer must be 'Received' in order to upload the discrepancy proof."
            );
        }
        ...
        try {
            ...
            Log::error('Store Manager Stock Transfer Discrepancy Proof', [
                'error_message' => $exception->getMessage(),
                'error_code' => 'Error code: ' . $exception->getCode(),
                'file' => 'File: ' . $exception->getFile(),
                'line' => 'Line: ' . $exception->getLine(),
    ...
    public function closeDiscrepancy(Request $request, int $stockTransferId): RedirectResponse
    {
        ...
        if ($stockTransfer->getStatus() !== $discrepancyStatus) {
            throw new RedirectBackWithErrorException(
                "The Stock Transfer status is no longer 'Discrepancy'. It cannot be closed from the discrepancy page."
            );
        }
        ...
        if ($anyReceivedQuantityPending) {
            throw new RedirectBackWithErrorException(
                'Received quantity is not set for one of the stock transfer items.'
            );
        }
        ...
        try {
            ...
            Log::error('Store-Manager-Stock-Transfer-Close-Discrepancy', [
                'error_message' => 'Error message: ' . $throwable->getMessage(),
                'error_code' => 'Error code: ' . $throwable->getCode(),
                'file' => 'File: ' . $throwable->getFile(),
                'line' => 'Line: ' . $throwable->getLine(),
    ...
    public function exportStockTransfers(string $filename, Request $request): BinaryFileResponse
    {
        ...
        $stockTransfers = $this->stockTransferQueries->getStoreManagerStockTransfersExport(
            $filterData,
            session('store_manager_selected_location_company_id'),
            session('store_manager_selected_location_id'),
        );
    ...
    public function updateAdditionalItems(Request $request, int $stockTransferId): void
    {
        ...
        try {
            /** @var User $storeManager */
            $storeManager = $request->user();

            $stockTransferService->updateAdditionalItems($additionalItems, $storeManager, $derivatives);

            DB::commit();
        } catch (Throwable $throwable) {
            Log::error('Store Manager Additional item received', [
                'error_message' => 'Error message: ' . $throwable->getMessage(),
                'error_code' => 'Error code: ' . $throwable->getCode(),
                'file' => 'File: ' . $throwable->getFile(),
                'line' => 'Line: ' . $throwable->getLine(),
    ...
    public function deliveryNoteItemRemarks(Request $request, int $stockTransferItemId): void
    {
        ...
        $stockTransferService = resolve(StockTransferService::class);
        $stockTransferService->deliveryNoteItemRemarks($storeManager, $validatedData['remarks'], $stockTransferItemId);
    }
    ...
    private function getTransferInAndOutStatusCountsByTypeAndFilter(
        ...
        if (array_key_exists(
            'dashboard_transfer_type',
            $filterData
        ) && null !== $filterData['dashboard_transfer_type']) {
            if ((int) $filterData['dashboard_transfer_type'] === StockTransferTypes::TRANSFER_ORDER->value) {
                $transferType = [StockTransferTypes::TRANSFER_ORDER->value];
            }

            if ((int) $filterData['dashboard_transfer_type'] === StockTransferTypes::REQUEST_ORDER->value) {
                $transferType = [StockTransferTypes::REQUEST_ORDER->value];
            }
        }

        $results = $this->stockTransferQueries->storeManagerTransferOrderStatusCount(
            $transferType,
            $filterData,
            $companyId,
            $locationId,
    ...
    private function getTransferOrRequestStatusCountsByTypeAndFilter(
        ...
        $results = $this->stockTransferQueries->storeManagerTransferOrRequestOrderStatusCount(
            $transferType,
            $filterData,
            $companyId,
            $locationId,
    ...
    private function markAsRejected(
        ...
        if ($stockTransfer->transfer_type === StockTransferTypes::TRANSFER_ORDER->value) {
            ...
            throw new RedirectBackWithErrorException(
                'The selected location does not have the authority to mark it as rejected'
            );
        }
    }
    ...
    private function isDestinationLocationInTransferOrder(StockTransfer $stockTransfer, int $locationId): bool
    {
        ...
        if (LocationTypes::STORE->value !== $destinationLocation->type_id) {
            return false;
        }
