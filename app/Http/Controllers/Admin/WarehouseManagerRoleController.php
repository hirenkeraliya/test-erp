...
class WarehouseManagerRoleController extends Controller
{
    ...
    {
        return Inertia::render('warehouse_manager_roles/Index');
    }
    ...
    {
        return Inertia::render('warehouse_manager_roles/Manage', [
    ...
    public function store(Request $request): RedirectResponse
    {
        $validatedData = $request->validate([
            'name' => ['required', 'string', 'unique:roles,name,NULL,id,guard_name,warehouse_manager'],
            'permissions' => ['required', 'array'],
        ]);

        $validatedData['guard_name'] = 'warehouse_manager';
        $this->roleQueries->store($validatedData);

        return to_route('admin.warehouse_manager_roles.index')->with(
            'success',
            'Roles & Permissions added successfully.'
        );
    }
    ...
    public function edit(int $roleId): Response
    {
        ...
        return Inertia::render('warehouse_manager_roles/Manage', [
            'role' => [
                'id' => $role->id,
                'name' => $role->name,
                'permissions' => WarehouseManagerPermissionModuleService::preparedEditRecord($role),
            ],
            'permissions' => WarehouseManagerPermissionModuleService::preparedPermissionModules(),
        ]);
    }
    ...
    {
        $validatedData = $request->validate([
            'name' => ['required', 'string', 'unique:roles,name,' . $roleId . ',id,guard_name,warehouse_manager'],
            'permissions' => ['required', 'array'],
        ]);
    ...
    public function clone(int $roleId): Response
    {
        ...
        return Inertia::render('warehouse_manager_roles/Manage', [
            'cloneRole' => [
                'id' => $cloneRole->id,
                'name' => $cloneRole->name,
                'permissions' => WarehouseManagerPermissionModuleService::preparedEditRecord($cloneRole),
            ],
            'permissions' => WarehouseManagerPermissionModuleService::preparedPermissionModules(),
        ]);
    }
