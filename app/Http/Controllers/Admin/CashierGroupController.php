...
class CashierGroupController extends Controller
{
    ...
    public function fetchCashierGroups(Request $request): array
    {
        ...
        $lengthAwarePaginator = $this->cashierGroupQueries->listQuery($filterData, session('admin_company_id'));
    ...
    public function store(CashierGroupData $cashierGroupData, Request $request): RedirectResponse
    {
        ...
        try {
            ...
            $this->cashierGroupQueries->addNew($cashierGroupData, session('admin_company_id'), $user);

            DB::commit();

            return to_route('admin.cashier_groups.index')->with('success', 'Cashier group added successfully.');
        } catch (Throwable $throwable) {
            Log::error('Add Cashier Group', [
    ...
    public function update(CashierGroupData $cashierGroupData, int $cashierGroupId): RedirectResponse
    {
        ...
        try {
            $this->cashierGroupQueries->update($cashierGroupData, $cashierGroupId, session('admin_company_id'));

            DB::commit();

            $this->removeFromCache($cashierGroupId);

            return to_route('admin.cashier_groups.index')->with('success', 'Cashier group updated successfully.');
        } catch (Throwable $throwable) {
            Log::error('Update Cashier Group', [
                'error_message' => $throwable->getMessage(),
                'error_code' => 'Error code: ' . $throwable->getCode(),
                'file' => 'File: ' . $throwable->getFile(),
                'line' => 'Line: ' . $throwable->getLine(),
    ...
    public function exportCashierGroups(string $filename, Request $request): BinaryFileResponse
    {
        ...
        $cashierGroups = $this->cashierGroupQueries->getCashierGroupsExport($filterData, session('admin_company_id'));
