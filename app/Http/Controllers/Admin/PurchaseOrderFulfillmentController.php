...
class PurchaseOrderFulfillmentController extends Controller
{
    ...
    public function deliveryOrder(int $purchaseOrderId): Response
    {
        ...
        $purchaseOrder = $purchaseOrderQueries->getByIdAndCompanyIdWithItems($purchaseOrderId, $companyId);
    ...
    public function shippingDetails(int $purchaseOrderId): Response|RedirectResponse
    {
        ...
        $purchaseOrder = $purchaseOrderQueries->getByPurchaseOrderIdForCreateDo($purchaseOrderId, $companyId);

        $purchaseOrderFulfillmentService = resolve(PurchaseOrderFulfillmentService::class);
        $routeUrl = 'admin.purchase_orders.index';
    ...
    public function shipped(Request $request, int $purchaseOrderFulfillmentId): RedirectResponse
    {
        ...
        $purchaseOrderFulfillment = $this->purchaseOrderFulfillmentQueries->getByIdWithRelation(
        ...
        try {
            $purchaseOrderFulfillmentService->shippedDeliveryOrder($purchaseOrderFulfillment, $admin);

            DB::commit();

            return to_route(
                'admin.purchase_order_fulfillments.delivery_order',
    ...
    public function markAsReceived(Request $request, int $purchaseOrderFulfillmentId): RedirectResponse
    {
        ...
        try {
            $purchaseOrderFulfillmentService->markAsReceivedDeliveryOrder($purchaseOrderFulfillment, $admin);
    ...
    public function markAsOpen(Request $request, int $purchaseOrderFulfillmentId): RedirectResponse
    {
        ...
        try {
            $purchaseOrderFulfillmentService->markAsOpen($purchaseOrderFulfillment, $admin);
            ...
            return to_route(
                'admin.purchase_order_fulfillments.delivery_order',
                $purchaseOrderFulfillment->purchaseOrder?->id
            )->with('success', 'Delivery Order details Open successfully');
        } catch (Throwable $throwable) {
            Log::error([
                'purchase order' => $throwable->getMessage(),
                'Full error' => [$throwable],
    ...
    public function markAsCancel(Request $request, int $purchaseOrderFulfillmentId): RedirectResponse
    {
        ...
        try {
            $purchaseOrderFulfillmentService->markAsCancelDeliveryOrder($purchaseOrderFulfillment, $admin);
    ...
    public function deliveryNote(int $purchaseOrderFulfillmentId): Response
    {
        ...
        $purchaseOrderFulfillmentService = resolve(PurchaseOrderFulfillmentService::class);
        $purchaseOrderFulfillmentService->checkDeliveryNote($purchaseOrderFulfillment, 'admin.purchase_orders.index');
        ...
        return Inertia::render('purchase_order_fulfillments/DeliveryNote', [
            ...
            'discrepancyTypes' => [
                ...
                'batch_discrepancy' => DiscrepancyTypes::BATCH_DISCREPANCY->value,
            ],
    ...
    public function closed(Request $request, int $purchaseOrderFulfillmentId): RedirectResponse
    {
        ...
        try {
            $purchaseOrderFulfillmentService->closedDeliveryOrder($purchaseOrderFulfillment, $purchaseOrder, $admin);

            DB::commit();

            return to_route('admin.purchase_order_fulfillments.delivery_order', $purchaseOrder->id)->with(
                'success',
    ...
    public function discrepancy(Request $request, int $purchaseOrderFulfillmentId): RedirectResponse
    {
        ...
        try {
            $purchaseOrderFulfillmentService = resolve(PurchaseOrderFulfillmentService::class);
            $purchaseOrderFulfillmentService->deliveryOrderDiscrepancy($purchaseOrderFulfillment, $admin);
    ...
    {
        $companyId = session('admin_company_id');
        $purchaseOrderFulfillment = $this->purchaseOrderFulfillmentQueries->getByIdWithRelationForEdit(
            $purchaseOrderFulfillmentId,
            $companyId
        );
    ...
    public function update(
        ...
        $companyId = session('admin_company_id');
        $purchaseOrderFulfillment = $this->purchaseOrderFulfillmentQueries->getByIdAndCompanyIdWithItems(
            $purchaseOrderFulfillmentId,
            $companyId
        );
    ...
    public function addShippingDetails(
        ...
        try {
            ...
            return to_route('admin.purchase_order_fulfillments.delivery_order', $purchaseOrderId)->with(
                'success',
    ...
    public function closeDiscrepancy(Request $request, int $purchaseOrderFulfillmentId): RedirectResponse
    {
        ...
        if ($anyReceivedQuantityPending) {
            throw new RedirectBackWithErrorException('one of the Delivery Order item received quantity pending.');
        }
        ...
        try {
            $purchaseOrderFulfillmentService->closeDeliveryOrderDiscrepancy($purchaseOrderFulfillment, $admin);
    ...
    public function print(int $purchaseOrderFulfillmentId): string
    {
        ...
        return $purchaseOrderFulfillmentPrintService->print($purchaseOrderFulfillmentId, session('admin_company_id'));
    }
