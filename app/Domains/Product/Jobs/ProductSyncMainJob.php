...
class ProductSyncMainJob implements ShouldQueue
{
    use Dispatchable;
    ...
    public function handle(): void
    {
        ...
        foreach ($saleChannels as $saleChannel) {
            $firstProduct = $productQueries->getFirstForEcommerceSync($this->companyId, $saleChannel->id);
            ...
            $lastProduct = $productQueries->getLastForEcommerceSync($this->companyId, $saleChannel->id);
            ...
            for ($startId = $firstProduct->id; $startId <= $lastProduct->id; $startId += 100) {
                $endId = $startId + 99;

                $jobs[] = new ProductSyncJob($startId, $endId, $saleChannel->id, $this->companyId);
            }

            $jobs[] = new UpdateSyncStatusJob($saleChannel->id, SyncTypes::PRODUCT->value);
