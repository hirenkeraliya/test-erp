...
class LimitedToCategoriesPromotionService implements SalePromotionInterface
{
    public function checkForApplicability(
        ...
        $promotionCategoryIds = $promotion->categories->pluck('id');

        $isValidProductAccordingToCategories = $promotionCategoryIds->intersect($productCategoryIds);

        if ($isValidProductAccordingToCategories->isEmpty()) {
            $saleMismatchMessage = 'Specified promotion is not applicable on the given product ' . $product->name . '.';
