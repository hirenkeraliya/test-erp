...
class BuyTwoAndGetOneQuantityAtRM1PromotionService implements SalePromotionInterface
{
    public function checkForApplicability(
        ...
        if (! array_key_exists('group_id', $cartItem)) {
            $saleMismatchMessage = 'group id is required for Buy 2 and get 1 quantity at RM1 promotion but not passed.';
        ...
        if ($buyItems->count() !== $totalBuyItems) {
            $saleMismatchMessage = 'Some of the specified buy products do not match with our records for the Buy 2 and get 1 quantity at RM1 promotion.';
            CommonFunctions::addMismatchOrAbort($checkSaleDetailsService->saleMismatches, $saleMismatchMessage);
        }
        ...
        $totalGetItems = $getItems->whereIn('id', $promotion->regularProducts->pluck('id'))->count();

        if ($getItems->count() !== $totalGetItems) {
            $saleMismatchMessage = 'Some of the specified Get products do not match with our records for the Buy 2 and get 1 quantity at RM1 promotion.';
            CommonFunctions::addMismatchOrAbort($checkSaleDetailsService->saleMismatches, $saleMismatchMessage);
        }

        $totalGetQuantity = $getItems->sum('quantity');

        $applicableQuantities = $this->getTotalApplicableQuantities($totalBuyQuantity, $promotion);

        if ($totalGetQuantity > $applicableQuantities) {
            $saleMismatchMessage = 'Only ' . $applicableQuantities . ' units are eligible for the Buy 2 and get 1 quantity at RM1 promotion. But requested units are ' . $totalGetQuantity . '.';
            CommonFunctions::addMismatchOrAbort($checkSaleDetailsService->saleMismatches, $saleMismatchMessage);
        }
        ...
        if (CommonFunctions::compareFloatNumbers($discountAmount, (float) $cartItem['item_discount_amount'])) {
            return;
        }
    ...
    public function getTotalApplicableQuantities(
        ...
        $applicableQuantities += $promotionTier->get_quantity;
    ...
    public function getTotalDiscountAmount(
        ...
        return $this->getTotalDiscountAmount($promotion, $applicableItemTotal, $getQuantity, $totalBuyQuantity);
    }
