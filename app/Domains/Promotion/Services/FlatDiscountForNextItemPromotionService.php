...
class FlatDiscountForNextItemPromotionService implements SalePromotionInterface
{
    public function checkForApplicability(
        ...
        if (! array_key_exists('group_id', $cartItem)) {
            $saleMismatchMessage = 'group id is required for Flat Discount For Next Item promotions but not passed.';
        ...
        if (! array_key_exists('discount_item_sequence', $cartItem)) {
            $saleMismatchMessage = 'discount item sequence is required for Flat Discount For Next Item promotions but not passed.';
            CommonFunctions::addMismatchOrAbort($checkSaleDetailsService->saleMismatches, $saleMismatchMessage);
        ...
        if ($discountableItems->count() !== $totalDiscountableItems) {
            $saleMismatchMessage = 'Some of the specified discountable products do not match with our records for the Flat Discount For Next Item promotion.';
            CommonFunctions::addMismatchOrAbort($checkSaleDetailsService->saleMismatches, $saleMismatchMessage);
        }
    ...
    public function calculateItemDiscountAmount(
        ...
        $totalBuyQuantity = $groupItems->where('item_discount_amount', '>=', 0)
        ...
        foreach ($promotionTiers as $promotionTier) {
            $discount = CommonFunctions::numberFormat((float) $promotionTier->get_value);
