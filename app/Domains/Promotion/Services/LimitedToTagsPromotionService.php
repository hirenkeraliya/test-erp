...
class LimitedToTagsPromotionService implements SalePromotionInterface
{
    public function checkForApplicability(
        ...
        $promotionTagIds = $promotion->tags->pluck('id');

        $isValidProductAccordingToTags = $promotionTagIds->intersect($productTagIds);
        if ($isValidProductAccordingToTags->isEmpty()) {
            $saleMismatchMessage = 'Specified promotion is not applicable on the given product tag ' . $product->name . '.';
