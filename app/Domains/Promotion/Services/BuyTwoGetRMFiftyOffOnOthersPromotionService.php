...
class BuyTwoGetRMFiftyOffOnOthersPromotionService implements SalePromotionInterface
{
    public function checkForApplicability(
        ...
        if (! array_key_exists('group_id', $cartItem)) {
            $saleMismatchMessage = 'group id is required for Buy 2 get RM50 off on others promotions but not passed.';
        ...
        if ($discountableItems->count() !== $totalDiscountableItems) {
            $saleMismatchMessage = 'Some of the specified discounted products do not match with our records for the Buy 2 get RM50 off on others promotion.';
            CommonFunctions::addMismatchOrAbort($checkSaleDetailsService->saleMismatches, $saleMismatchMessage);
        }
        ...
        if ($buyItems->count() !== $totalBuyItems) {
            $saleMismatchMessage = 'Some of the specified buy products do not match with our records for the Buy 2 get RM50 off on others promotion.';
            CommonFunctions::addMismatchOrAbort($checkSaleDetailsService->saleMismatches, $saleMismatchMessage);
        }
        ...
        if ($applicableDiscountableQuantities < $totalDiscountableQuantities) {
            $saleMismatchMessage = 'Only ' . $applicableDiscountableQuantities . ' units can be given for free for this Buy 2 get RM50 off on others promotion. But requested discounted quantities are ' . $totalDiscountableQuantities . '.';
            CommonFunctions::addMismatchOrAbort($checkSaleDetailsService->saleMismatches, $saleMismatchMessage);
        }

        $discountAmount = $this->calculateItemDiscountAmount($promotion, $itemTotal, $buyItems);
    ...
    public function calculateItemDiscountAmount(
        ...
        $itemDiscount = $promotionTier->get_value;

        if ($itemTotal < $itemDiscount) {
            return CommonFunctions::numberFormat($itemTotal);
        }

        return CommonFunctions::numberFormat((float) $promotionTier->get_value);
    }
