...
class CityCreateListener
{
    public function handle(CityCreateEvent $cityCreateEvent): void
    {
        $city = $cityCreateEvent->city;
        ...
        $cityRetailPlanningIntegrationService->manageCity($city, IntegrationWebhookUrls::CITY_CREATE->value);

        $saleChannelQueries = resolve(SaleChannelQueries::class);

        $webhookUrls = [WebhookUrls::CITY_CREATE->value];
        ...
        Log::channel('e_commerce')->info('sale channel webhook city create started', [
            'start time of the webhook call for the city create' => Carbon::now()->format('Y-m-d H:i:s'),
            'city id: ' . $city->getKey(),
        ]);
        ...
        try {
            ...
            Log::channel('e_commerce')->error('sale channel webhook city create failed', [
        ...
        Log::channel('e_commerce')->info('sale channel webhook city create ended', [
            'end time of the webhook call for the city create' => Carbon::now()->format('Y-m-d H:i:s'),
            'city id: ' . $city->getKey(),
        ]);
    }
}
