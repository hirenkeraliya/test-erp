...
use App\Domains\StoreManager\Enums\StoreManagerImportColumns;
use App\Domains\StoreManager\Services\StoreManagerService;
use App\Domains\StoreManager\StoreManagerQueries;
use App\Models\ImportRecord;

class ImportStoreManager implements ImportRecordClassInterface
{
    /**
     * @return mixed[]
     */
    public function validate(array $storeManagerDetails, ImportRecord $importRecord): array
    {
        ...
        if ($storeManagerQueries->doExistsByStoreManagerUsername(
            (string) $storeManagerDetails['username'],
            $importRecord->company_id
        )) {
            $validationErrors[] = 'The specified username is already assigned as a store manager.';
        }

        if (! array_key_exists('password', $storeManagerDetails) || ! $storeManagerDetails['password']) {
            $validationErrors[] = 'The password is required.';
        }

        if (! array_key_exists('passcode', $storeManagerDetails) || ! $storeManagerDetails['passcode']) {
            $validationErrors[] = 'The passcode is required.';
        }
        ...
        if ($storeManagerQueries->doExistsByEmployeeId($employeeId)) {
            $validationErrors[] = 'The specified employee has already been assigned as a store manager';
        }
        ...
        if (
            ...
            || ! $storeManagerDetails['price_override_limit_percentage_for_cart'])
        ) {
    ...
    public function save(array $storeManagerDetails, ImportRecord $importRecord): void
    {
        ...
        $storeManagerDetails['password'] = bcrypt($storeManagerDetails['password']);
        ...
        $storeManagerQueries = resolve(StoreManagerQueries::class);
        $storeManagerQueries->addNew($storeManagerData);
    }

    public function validateColumns(array $uploadHeaderColumns, array $allowedPermissionLists, int $companyId): array
    {
        $requiredHeaderColumns = collect(StoreManagerImportColumns::cases())->pluck('value')->toArray();
