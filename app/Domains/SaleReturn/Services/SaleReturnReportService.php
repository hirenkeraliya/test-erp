...
namespace App\Domains\SaleReturn\Services;
...
use App\Domains\SaleReturn\Exports\SaleReturnReportExport;
...
class SaleReturnReportService
{
    public function print(int $companyId, array $filterData): string
    {
        ...
        return view('prints.sales_return', [
    ...
    public function export(int $companyId, array $filterData, string $filename): BinaryFileResponse
    {
        ...
        return Excel::download(new SaleReturnReportExport($locationsSales), $filename);
    }
    ...
    private function prepareData(array $filterData, int $companyId): array
    {
        ...
        $saleReturns = $saleReturnQueries->getForSaleReturnReport($filterData);
        ...
        foreach ($locations as $location) {
            ...
            foreach ($saleReturns->where('counterUpdate.counter.location_id', $location->id) as $saleReturn) {
    ...
    private function filterBy(array $filterData): string
    {
        ...
        if ($filterBy === SaleReturnFilterTypes::BY_COUNTER->value && isset($filterData['counter_ids']) && '' !== $filterData['counter_ids']) {
            $counters = $counterQueries->getByIds($filterData['counter_ids']);

            return $this->formatFilterResult(
                SaleReturnFilterTypes::BY_COUNTER->value,
                $counters->pluck('name')->implode(', ')
            );
        }

        if ($filterBy === SaleReturnFilterTypes::BY_CASHIER->value && isset($filterData['cashier_ids']) && '' !== $filterData['cashier_ids']) {
            $cashiers = $cashierQueries->getByIds($filterData['cashier_ids']);

            return $this->formatFilterResult(
                SaleReturnFilterTypes::BY_CASHIER->value,
    ...
    {
        return SaleReturnFilterTypes::getFormattedCaseName($filterType) . ' (' . $value . ')';
    }
}
