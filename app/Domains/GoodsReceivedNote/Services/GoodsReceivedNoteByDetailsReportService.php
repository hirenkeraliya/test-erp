...
class GoodsReceivedNoteByDetailsReportService
{
    public function preparedByDetails(array $filterData, Company $company, Collection $locations): string
    {
        [$goodsReceivedNotes, $columns, $dateRange] = $this->fetchRecords($filterData, $company, $locations);
    ...
    public function fetchRecords(array $filterData, Company $company, Collection $locations): array
    {
        ...
        $goodsReceivedNotes = $goodsReceivedNoteQueries->getByDateAndLocationsWithGRNProductAndProduct(
            $filterData,
            $company->id
        );
        ...
        $columns = ['Date', 'Location', 'Created By', 'Grn Ref', 'Do Ref', 'Po Ref', 'Quantity', 'Notes'];
        ...
        foreach ($locations as $location) {
            ...
            foreach ($selectedLocationGoodsReceivedNotes as $selectedLocationGoodReceivedNote) {
                ...
                $locationGoodsReceivedNotes['goods_received_notes'][] = [
                    ...
                    'products' => $this->preparedProductRecords($goodsReceivedNoteProducts),
                ];
            }
    ...
    private function preparedProductRecords(Collection $goodsReceivedNoteProducts): array
    {
        $groupBy = config('app.product_variant') ? 'product.masterProduct.article_number' : 'product.article_number';

        return $goodsReceivedNoteProducts->groupBy($groupBy)->map(fn ($items): array => [
