...
use App\Domains\Cashier\Enums\CashierImportColumns;
...
class ImportCashier implements ImportRecordClassInterface
{
    /**
     * @return mixed[]
     */
    public function validate(array $cashierDetails, ImportRecord $importRecord): array
    {
        ...
        if ($cashierQueries->doExistsByCashierUsername(
            (string) $cashierDetails['username'],
            $importRecord->company_id
        )) {
            $validationErrors[] = 'The specified username is already assigned as a cashier.';
        }

        if (! array_key_exists('pin', $cashierDetails) || ! $cashierDetails['pin']) {
            $validationErrors[] = 'The pin is required.';
        }
        ...
        if ($cashierQueries->doExistsByEmployeeId($employeeId)) {
            $validationErrors[] = 'The specified employee has already been assigned as a cashier';
        }
    ...
    public function save(array $cashierDetails, ImportRecord $importRecord): void
    {
        ...
        $cashierQueries = resolve(CashierQueries::class);
        $cashierQueries->addNew($cashierData, $importRecord->createdBy);
    }

    public function validateColumns(array $uploadHeaderColumns, array $allowedPermissionLists, int $companyId): array
    {
        $requiredHeaderColumns = collect(CashierImportColumns::cases())->pluck('value')->toArray();
