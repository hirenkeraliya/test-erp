...
class PrintLayawaySaleTaxInvoiceService
{
    public function print(int $saleId, int $companyId, ?int $locationId = null): string
    {
        $saleQueries = resolve(SaleQueries::class);
        $productService = resolve(ProductService::class);

        $sales = $saleQueries->getLayawaySaleItemsByForPrint($saleId, $companyId, $locationId);

        $saleDetails = $this->preparedData($sales, $productService);
        [$company, $location] = $this->preparedCompanyAndStore($sales);

        $currencyQueries = resolve(CurrencyQueries::class);
        $currency = $currencyQueries->getByCompanyId($companyId);

        return view('prints.layaway_sale_tax_invoice', [
