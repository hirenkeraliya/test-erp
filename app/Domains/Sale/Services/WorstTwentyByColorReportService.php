...
use App\Domains\Sale\Exports\WorstTwentyByColorExport;
...
class WorstTwentyByColorReportService
{
    public function print(int $companyId, array $filterData): string
    {
        [$locationsSales, $company] = $this->fetchWorstTwentyColorRecords($companyId, $filterData);

        $customReportService = resolve(CustomReportService::class);

        return view('prints.worst_twenty_by_colors', [
    ...
    public function export(int $companyId, array $filterData, string $filename): BinaryFileResponse
    {
        ...
        return Excel::download(
            new WorstTwentyByColorExport($locationsSales, $dateRange, $company, $filterBy, $displayAmount),
            $filename
        );
    }

    private function fetchWorstTwentyColorRecords(int $companyId, array $filterData): array
    {
        ...
        $saleItems = $saleItemQueries->getByStoreForTopColorExport($filterData);
                ...
                foreach ($colors as $colorData) {
                    ...
                    foreach ($filteredSaleItems->where('product.color_id', $color->id)->groupBy(
                        ...
                        if ($product->color_id === $color->id) {
                            ...
                            $saleColors['products'][$productId]['discount_amount'] += $saleItem->sum(
                            ...
                            $saleColors['products'][$productId]['gst_amount'] += $saleItem->sum('total_tax_amount');
                    ...
                    $saleColorProducts = collect($saleColors['products'])->sortBy('qty')->take(20)->values();
            ...
            foreach ($colors as $colorData) {
                ...
                foreach ($filteredSaleItems->where('product.color_id', $color->id) as $saleItem) {
                    ...
                    if ($product->color_id === $color->id) {
                        ...
                        $saleColors['products'][$productId]['gross_sales_excl_gst'] += ($saleItem->total_price_paid - $saleItem->total_tax_amount);
                        $saleColors['products'][$productId]['discount_amount'] += $saleItem->total_discount_amount;
                        $saleColors['products'][$productId]['net_sales_excl_gst'] += ($saleItem->total_price_paid - $saleItem->total_tax_amount - $saleItem->total_discount_amount);
                        $saleColors['products'][$productId]['gst_amount'] += $saleItem->total_tax_amount;
                        $saleColors['products'][$productId]['net_sales_incl_gst'] += $saleItem->total_price_paid;
                    }
                }
        ...
        foreach ($locations as $location) {
            ...
            foreach ($colors as $colorData) {
                ...
                foreach ($filteredSaleItems->where('product.style_id', $color->id) as $saleItem) {
                    $product = $saleItem->product;
