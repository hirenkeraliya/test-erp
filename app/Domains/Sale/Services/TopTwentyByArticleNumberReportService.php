...
use App\Domains\Sale\Exports\TopTwentyByArticleNumberExport;
use App\Domains\TopTwentyAggregateData\TopTwentyAggregateDataQueries;
use Carbon\Carbon;
use Maatwebsite\Excel\Facades\Excel;
use Symfony\Component\HttpFoundation\BinaryFileResponse;

class TopTwentyByArticleNumberReportService
{
    public function print(int $companyId, array $filterData): string
    {
        [$locationsSales, $company] = $this->fetchTopTwentyArticleNumberRecords($companyId, $filterData);

        $customReportService = resolve(CustomReportService::class);

        return view('prints.top_twenty_by_article_numbers', [
    ...
    public function export(int $companyId, array $filterData, string $filename): BinaryFileResponse
    {
        ...
        return Excel::download(
            new TopTwentyByArticleNumberExport($locationsSales, $dateRange, $company, $filterBy, $displayAmount),
            $filename
        );
    }

    private function fetchTopTwentyArticleNumberRecords(int $companyId, array $filterData): array
    {
        ...
        $articleNumbers = $topTwentyAggregateData->sortByDesc('quantity')->pluck(
        ...
        if ($filterData['combine_stock_by_selected_location']) {
            ...
            foreach ($articleNumbers as $articleNumber) {
                ...
                foreach ($filteredSaleItems->where(
                    ...
                    $productArticleNumber = config('app.product_variant')
                    ...
                    if ($productArticleNumber === $articleNumber) {
                        ...
                        $saleByArticleNumber['products'][$productId]['gross_sales_excl_gst'] += $saleItem->gross_sales;
                        $saleByArticleNumber['products'][$productId]['discount_amount'] += $saleItem->discount;
                        $saleByArticleNumber['products'][$productId]['net_sales_excl_gst'] += $saleItem->net_sales;
                        $saleByArticleNumber['products'][$productId]['gst_amount'] += $saleItem->tax;
                        $saleByArticleNumber['products'][$productId]['net_sales_incl_gst'] += $saleItem->total_amount;
                    }
                }

                $saleByArticleNumberProducts = collect($saleByArticleNumber['products'])->sortByDesc('qty')->take(
                    20
                )->values();
