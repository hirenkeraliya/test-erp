...
class PrintCancelLayawaySaleReportService
{
    public function print(int $saleId, int $companyId, ?int $locationId): string
    {
        ...
        $layawaySaleDetails = $saleQueries->getCancelLayawaySaleItemsByForPrint($saleId, $companyId, $locationId);

        $salesDetails = $this->preparedData($layawaySaleDetails, $productService);
        [$company, $location] = $this->preparedCompanyAndStore($layawaySaleDetails);

        $currencyQueries = resolve(CurrencyQueries::class);
        $currency = $currencyQueries->getByCompanyId($companyId);

        return view('prints.cancel_layaway_sale_report', [
    ...
    private function preparedData(Sale $sale, ProductService $productService): array
    {
        ...
        $cancelLayawaySale = $sale->cancelLayawaySale;

        /** @var CreditNote $creditNote */
        $creditNote = $cancelLayawaySale->creditNote;
        ...
        return [
            ...
            'offline_sale_id' => $sale->offline_sale_id ?? 'N/A',
            'bill_reference_number' => $sale->bill_reference_number ?? 'N/A',
    ...
    private function getPreparedSaleItems(Collection $saleItems, ProductService $productService): array
    {
        return $saleItems->map(function ($saleItem) use ($productService): array {
            ...
            return [
                ...
                'total_pending_layaway_amount' => $saleItem->calculateFinalSaleItemAmount() - $saleItem->getTotalPricePaid(),
