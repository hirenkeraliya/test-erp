...
class ImportColorBulkUpdate implements ImportRecordClassInterface
{
    public function validate(array $colorDetails, ImportRecord $importRecord): array
    {
        ...
        if (! array_key_exists('name', $colorDetails) || ! $colorDetails['name']) {
            ...
            $validationErrors[] = 'The specified color is not available in our records.';
        }

        if (array_key_exists(
            'code',
            $colorDetails
        ) && null !== $colorDetails['code'] && $colorQueries->codeTakenByAnotherColor(
            (string) $colorDetails['code'],
            (string) $colorDetails['name'],
            $importRecord->company_id
        )) {
            $validationErrors[] = 'Specified color code is already assign';
        }
    ...
    public function save(array $colorDetails, ImportRecord $importRecord): void
    {
        ...
        $colorQueries->updateByName($colorData->all(), (string) $colorDetails['name'], $importRecord->company_id);
    }
