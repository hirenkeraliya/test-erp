...
class ImportVendorBulkUpdate implements ImportRecordClassInterface
{
    /**
     * @return string[]
     */
    public function validate(array $vendorDetails, ImportRecord $importRecord): array
    {
        ...
        } elseif ($vendorQueries->existsByNameExpectCurrentRecord(
            (string) $vendorDetails['name'],
            (string) $vendorDetails['phone'],
        ...
        } elseif (! filter_var($vendorDetails['email'], FILTER_VALIDATE_EMAIL)) {
            $validationErrors[] = 'Specified email is not valid';
        }
    ...
    public function save(array $vendorDetails, ImportRecord $importRecord): void
    {
        ...
        $vendorQueries = resolve(VendorQueries::class);
        $vendorQueries->updateByPhone($vendorData->all(), $importRecord->company_id);
    }
