...
use App\Domains\StockMovement\Exports\StockMovementByBrandExport;
...
class StockMovementByBrandServices
{
    public function fetchStockMovementDetailsByBrand(array $filterData, int $companyId): array
    {
        $sellThroughAggregateQueries = resolve(SellThroughAggregateQueries::class);
        $brandSalesData = $sellThroughAggregateQueries->stockMovementSummaryForBrand($filterData, $companyId);

        $lengthAwarePaginator = new LengthAwarePaginator(
            $brandSalesData->forPage($filterData['page'], $filterData['per_page']),
            $brandSalesData->count(),
            $filterData['per_page'],
            $filterData['page']
        );
        ...
        return [
            'data' => StockMovementByBrandListResource::collection($lengthAwarePaginator->values()),
            'total' => $stockMovementService->getBadgesTotal($brandSalesData),
    ...
    public function printStockMovementDetailsByBrand(
        array $filterData,
        int $companyId,
        ...
        return view('prints.stock_movement_by_brand', [
            'stockMovementDataByBrands' => StockMovementByBrandListResource::collection($brandSalesData),
            'stockMovementTotalDataByBrands' => [
                'name' => 'Grand Total',
                'sold' => $brandSalesData->sum('sold'),
                'remaining' => $brandSalesData->sum('balance'),
            ],
    ...
    public function exportStockMovementDetailsByBrand(
        array $filterData,
        int $companyId,
        string $filename,
        array $getFilterLabels
        ...
        return Excel::download(
            new StockMovementByBrandExport(
                $brandSalesData,
                $totals,
                $company,
                $locations,
