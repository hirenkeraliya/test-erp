<template>
    <div class="top-bar-boxed h-[120px] sm:h-[80px] md:h-[120px] lg:h-[65px] z-[51] border-b border-white/[0.08] mt-16 md:mt-0 -mx-3 sm:-mx-8 md:-mx-0 px-3 md:border-b-0 relative md:fixed md:inset-x-0 md:top-0 sm:px-8 md:px-10 md:pt-10 md:bg-gradient-to-b md:from-slate-100 md:to-transparent">
        <div class="h-auto sm:h-auto md:h-full flex items-center">
            <Link
                :href="route('store_manager.dashboard')"
            ...
            <Notification
                mark-all-as-read-url="store_manager.notifications.mark_all_as_read"
                fetch-url="store_manager.notifications.fetch"
                fetch-read-notification-url="store_manager.notifications.fetch_read_notification"
                mark-as-read-url="store_manager.notifications.mark_as_read"
                mark-as-unread-url="store_manager.notifications.mark_as_unread"
            />
...
import menus from '@storeManager/navbar';
...
const editProfile = (dismiss) => {
    router.get(route('store_manager.edit_profile'));
    dismiss();
};

const logout = (dismiss) => {
    ObjectStorage.remove('store-manager-store-id');
    router.post(route('store_manager.logout'));
    dismiss();
};
...
const state = reactive({
    ...
    localStorageName: 'store_manager_recently_visited_page',
});
...
const setLocationId = (selectedLocation) => {
...
onMounted(() => {
    axios
        .get(route('store_manager.get_authorized_stores'))
        .then(function (response) {
            state.locations = response.data.locations;
            const locationId = ObjectStorage.get('store-manager-store-id');
