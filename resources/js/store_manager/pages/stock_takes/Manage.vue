<template>
    ...
    <div class="intro-y xl:flex items-center mt-8">
        ...
        <div class="w-full sm:w-auto lg:flex mt-4 xl:mt-0">
            ...
            <JBadge
                :label="'Submitted Stock: ' + state.grandTotalSubmittedStock"
                class="mb-2 lg:mb-0"
            />

            <Link :href="route('store_manager.stock_takes.index')">
                <SecondaryButton
                    type="button"
                    text="Save as Draft"
    ...
    <div class="mt-10">
        <JTable
            :fetch-url="route('store_manager.stock_takes.fetch_stock_take_products', props.stockTakeId)"
    ...
    <AdvanceProductSelectionModalForStockTake
        ...
        product-article-search-url="store_manager.products.search_products_by_article_number"
...
const exportCsvRecord = (stockTakeId, params) => {
    return exportRecords(
        '/store-manager/download-stock-take-products/' + stockTakeId + '/',
...
const updateSubmittedStock = (submittedStock, stockTakeProductId, productId) => {
    ...
    axios.post(route('store_manager.stock_takes.update_submitted_stock', props.stockTakeId), {
...
const submitStockTake = () => {
    ...
    router.post(route('store_manager.stock_takes.submit', props.stockTakeId), {
...
const submitBulkUpdates = () => {
    ...
    stockTakeForm.post(route('store_manager.stock_takes.bulk_update_stocks', props.stockTakeId), {
        onSuccess: (page) => {
            ...
            router.get(route('store_manager.stock_takes.index'));
        }
    });
};

const getPendingStockProductsSubmissionCount = () => {
    axios.get(route('store_manager.stock_takes.get_pending_stock_product_submission_count', props.stockTakeId)).then((response) => {
...
const advanceFilterProductSelection = (selectedProducts) => {
    ...
    axios.post(route('store_manager.stock_takes.update_submitted_stock_by_stock_id', props.stockTakeId), {
...
const getGrandTotalSubmittedStock = () => {
    axios.get(route('store_manager.stock_takes.grand_total_submitted_stock', props.stockTakeId))
        .then((response) => {
