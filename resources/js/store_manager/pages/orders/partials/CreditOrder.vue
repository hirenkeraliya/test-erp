    ...
    >
        ...
        <ModalBody class="p-5 sm:p-10">
            <div class="text-lg font-medium flex justify-between">
                ...
                <p>
                    Due: {{ order.credit_pending_amount }}
                </p>
            </div>
            ...
            <div class="mt-5">
                ...
                <PrimaryButton
                    type="button"
                    text="Complete Credit Order"
                    class="ml-2"
                    @click="completeCreditOrder"
                />
            </div>
        </ModalBody>
    </Modal>
...
const creditOrderForm = useForm({
    paymentTypes: [],
    orderId: props.order.id,
});

const emits = defineEmits(['credit-order-close-modal']);

const hideCreditOrderModal = () => {
    emits('credit-order-close-modal');
};

const completeCreditOrder = () => {
    creditOrderForm.paymentTypes = state.paymentTypes.filter((paymentType) => paymentType.value !== null).map((paymentType) => {
        return {
    ...
    creditOrderForm.post(route('store_manager.orders.complete_credit_order'), {
        onSuccess: () => hideCreditOrderModal(),
    });
};
...
const updatePaymentTypeAmount = (amount, index) => {
    if (parseFloat(amount) >= parseFloat(props.order.credit_pending_amount)) {
        state.isPaymentTypeSelectBoxDisabled = true;
        state.paymentTypes[index].amount = props.order.credit_pending_amount;
        return;
    }
...
const isPaymentTypeSelectBoxDisabled = computed(() => {
    ...
    return parseFloat(paymentTypeAmount) >= parseFloat(props.order.credit_pending_amount);
});
</script>
