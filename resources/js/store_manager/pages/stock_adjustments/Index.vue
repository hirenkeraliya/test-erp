<template>
    ...
    <JTable
        :fetch-url="route('store_manager.stock_adjustments.fetch')"
...
const openStockAdjustmentItemsModal = (stockAdjustmentId) => {
    ...
    axios.get(route('store_manager.stock_adjustments.fetch_items', stockAdjustmentId))
        .then((response) => {
...
const refreshPage = () => {
    router.get(route('store_manager.stock_adjustments.index'));
};
