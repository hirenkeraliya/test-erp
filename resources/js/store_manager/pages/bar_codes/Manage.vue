<template>
            ...
            <div class="intro-y box">
                ...
                <div class="w-full p-5">
                                ...
                                <tbody>
                                    ...
                                    >
                                        <td class="whitespace-nowrap w-4/12">
                                            <JProductFilter
                                                :product-search-url="route('store_manager.get_filtered_products')"
                    ...
                    <div class="flex flex-row ml-auto">
                        <Link :href="route('store_manager.barcode_prints.index')">
                            <SecondaryButton
                                type="button"
                                text="Cancel"
                                class="w-24 mt-5"
    ...
    <JProductFilterDetails
        ...
        :product-search-url="route('store_manager.get_filtered_products_list')"
...
import { isPrintRecords } from '@commonServices/helper';
    ...
    if (isPrintRecords(props.exportPermission)) {
        if (barcodePrintForm.module_type === props.moduleTypeStaticEnum.manual) {
            if (state.barcodeCountThresholdForAsyncPrint === null ||
                ...
                axios.post(route('store_manager.barcode_prints.products_barcode_print_manual'), barcodePrintForm)
                    .then((response) => {
            ...
            if (state.barcodeCountThresholdForAsyncPrint && state.barcodeTotalQuantity > state.barcodeCountThresholdForAsyncPrint) {
                const message = 'Barcode will be prepared in the background because you reached the maximum limit. Are You Sure?';

                confirmDialogBox(message, () => {
                    barcodePrintForm.post(route('store_manager.barcode_prints.products_barcode_print'));
                });
            }
            return;
        }
