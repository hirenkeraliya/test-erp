<template>
    ...
    <JTable
        :fetch-url="route('store_manager.order_picking_lists.fetch')"
...
import OrderPickingListDetails from '@storeManagerPages/order_picking_lists/partials/OrderPickingListDetails.vue';
...
const showOrderDetailsModal = (orderPickingId) => {
    ...
    axios.get(route('store_manager.order_picking_lists.fetch_order_picking_list_items', orderPickingId))
        .then((response) => {
...
const printOrderPackaging = (orderPickingListId) => {
    printReport(route('store_manager.order_picking_lists.print_order_packaging', orderPickingListId));
};

const printOrderPackingList = (orderPickingListId) => {
    printReport(route('store_manager.order_picking_lists.print_order_packing_list', orderPickingListId));
};

const printNinjaVanWayBill = (orderPickingListId) => {
    printReport(route('store_manager.order_picking_lists.print_ninja_van_way_bills', orderPickingListId));
};
...
const inProgress = (orderPickingListId, dismiss) => {
    ...
    confirmDialogBoxWithCenterText('Are you sure? You want to mark this picking list as in-progress.', () => {
        router.post(route('store_manager.order_picking_lists.inprogress', orderPickingListId), {}, {
...
const cancel = (orderPickingListId, dismiss) => {
    ...
    confirmDialogBoxWithCenterText('Are you sure? You want to mark this picking list as cancelled.', () => {
        router.post(route('store_manager.order_picking_lists.cancel', orderPickingListId), {}, {
            onSuccess: () => refreshTable()
        });
    });
};
...
const completed = (orderPickingListId, dismiss) => {
    ...
    confirmDialogBoxWithCenterText('Are you sure? You want to mark this picking list as completed.', () => {
        router.post(route('store_manager.order_picking_lists.completed', orderPickingListId), {}, {
            onSuccess: () => refreshTable()
        });
    });
};
