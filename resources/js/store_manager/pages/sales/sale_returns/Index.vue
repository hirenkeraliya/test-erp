<template>
    ...
    <JTable
        ...
        :fetch-url="route('store_manager.sale_returns.fetch')"
...
const showSaleReturnDetailsModal = (saleReturnId) => {
    state.saleReturn = [];
    axios.get(route('store_manager.sale_returns.fetch_sale_return_items', saleReturnId))
        .then((response) => {
...
const refreshPage = () => {
    router.get(route('store_manager.sale_returns.index'));
};

onMounted(() => {
    if (props.offlineSaleReturnId) {
        state.isClear = true;
        state.displaySaleReturnFilter = true;
        refreshTable();
    }
});
