<template>
    ...
    <JTable
        ...
        :fetch-url="route('store_manager.products_report.fetch')"
        ...
        local-storage-key="store-manager-products-reports-columns"
...
const exportPDFRecords = (params, columns) => {
    ...
    printReport(route('store_manager.products_report.print_products_report', params), props.exportPermission);
};
...
const refreshPage = () => {
    router.get(route('store_manager.products_report.index'));
};

onMounted(() => {
    if (props.dashboardFilterData.product_id || props.dashboardFilterData.color_ids) {
        state.isClear = true;
