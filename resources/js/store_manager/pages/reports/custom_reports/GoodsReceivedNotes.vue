...
const updateTheFilterBy = (filterBy) => {
    ...
    if (filterBy === props.goodsReceivedNoteFilterStaticDetails.byVendor) {
        axios.get(route('store_manager.vendors.get_vendors_list'))
            .then((response) => {
...
const exportGoodsReceivedNotes = () => {
    if (validationCheck()) {
        showErrorNotification('Please select a report by and date before proceeding.');
        return;
    }
    ...
    printReport(route('store_manager.custom_reports.print_goods_received_note', state.parameters));
};
...
const exportExcelRecord = () => {
    ...
    if (state.parameters.filter_by === props.goodsReceivedNoteFilterStaticDetails.byBrand && state.parameters.brand_ids === null) {
        showErrorNotification('Please select brand.');
        return;
    }
    ...
    if (state.parameters.filter_by === props.goodsReceivedNoteFilterStaticDetails.byDepartment && state.parameters.department_ids === null) {
        showErrorNotification('Please select departments.');
        return;
    }
    ...
    if (state.parameters.filter_by === props.goodsReceivedNoteFilterStaticDetails.byVendor && state.parameters.vendor_ids === null) {
        showErrorNotification('Please select vendors.');
        return;
    }
