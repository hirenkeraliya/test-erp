...
const exportSalesCollection = () => {
    ...
    printReport(route('store_manager.custom_reports.print_discount_summary_report', state.parameters));
};
...
onMounted(() => {
    axios.get(route('store_manager.custom_reports.get_discount_type_reports'))
        .then((response) => {
            state.discountTypeReports = response.data.discountTypeReports;
        });
    axios.get(route('store_manager.custom_reports.get_sale_discount_type_reports'))
        .then((response) => {
