<template>
    <div class="py-5 md:py-0 overflow-x-hidden">
        <MobileMenu
            ...
            :home-url="route('store_manager.dashboard')"
            :menus="menus"
...
<script setup>
import TopBar from '@storeManagerComponents/TopBar.vue';
...
onMounted(() => {
    ...
    if (!ObjectStorage.get('store-manager-store-id')) {
        router.get(route('store_manager.store_selection'));
    }
});
...
const redirectToRoute = (menu) => {
    ...
    return route('store_manager.menu_page', { pageUrl: menu.title });
};

const isDashboardRoute = () => {
    const dashboardRoutes = [
        'store_manager.dashboard',
        'store_manager.revenue_view',
        'store_manager.store_revenue',
        'store_manager.business_view',
        'store_manager.stock_overview',
    ];
