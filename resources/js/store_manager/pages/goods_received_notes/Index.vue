<template>
    ...
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        ...
        >
            <Link :href="route('store_manager.goods_received_notes.create')">
                <PrimaryButton
    ...
    <JTable
        :fetch-url="route('store_manager.goods_received_notes.fetch')"
            ...
            <div v-if="record.item.upload_status !== importRecordStatus.completed && record.item.upload_status !== 'N/A'">
                <span class="flex flex-col gap-2">
                    <span>
                        Check Upload Status
                        <Link
                            :href="route('store_manager.import_records.index', record.item.import_record.id)"
    ...
    <ReUploadFailedImportRecordFile
        ...
        fetch-pending-statuses-count-url="store_manager.goods_received_notes.re_upload_goods_received_note_record"
        product-matching-upc-url="store_manager.products.get_matching_upc_inventory_products"
    ...
    <GoodsReceivedNoteCancelRemarks
        ...
        route-url="store_manager.goods_received_notes.mark_as_cancel"
...
import { Download, FileUp, FileX2, Info, List, Printer } from 'lucide-vue-next';
...
const openGoodsReceivedNoteProductsModal = (data) => {
    ...
    axios.get(route('store_manager.goods_received_notes.products', data.id))
        .then((response) => {
...
const printGoodsReceivedNote = (goodsReceivedNoteId) => {
    printReport(route('store_manager.goods_received_notes.goods_received_note_print', goodsReceivedNoteId), props.exportPermission);
};
...
const refreshPage = () => {
    router.get(route('store_manager.goods_received_notes.index'));
};
...
const fetchGoodsReceivedNoteImportRecordsPendingStatuses = () => {
    ...
    axios.get(route('store_manager.import_records.get_import_record_pending_statuses', props.goodsReceivedNoteModelMappingType))
        .then(response => {
