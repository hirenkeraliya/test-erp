<template>
    ...
    >
        <ProductListFilters
            ...
            :categories-url="route('warehouse_manager.categories.get_filtered_categories')"
            :brands-url="route('warehouse_manager.brands.get_filtered_brands')"
            :sizes-url="route('warehouse_manager.sizes.get_filtered_sizes')"
            :colors-url="route('warehouse_manager.colors.get_filtered_colors')"
            :departments-url="route('warehouse_manager.departments.get_filtered_departments')"
            :article-numbers-url="route('warehouse_manager.products.get_filtered_article_number')"
            :styles-url="route('warehouse_manager.styles.get_filtered_styles')"
            :tags-url="route('warehouse_manager.tags.get_filtered_tags')"
            :product-collections-url="route('warehouse_manager.product_collections.get_filtered_product_collection')"
    ...
    <JTable
        ...
        :fetch-url="route('warehouse_manager.products.fetch')"
        ...
        local-storage-key="warehouse-manager-products-columns"
...
import { ChevronRight, View, Album, FileClock } from 'lucide-vue-next';
...
const exportCsvRecords = () => {
    ...
    return axios.get(route('warehouse_manager.products.check_product_export_limit', state.parameters))
        .then((response) => {
...
const exportExcelRecordsForBulkProductUpdate = () => {
    state.isExportFileInProgress = true;
    return axios.get(route('warehouse_manager.products.check_product_export_limit_for_import_bulk_update', state.parameters))
        .then((response) => {
            if (!response.data.exceeds_limit) {
                state.isExportFileInProgress = false;
                return exportRecords(
...
const exportLoyaltyPointCsvRecords = () => {
    state.isExportFileInProgress = true;
    return axios.get(route('warehouse_manager.products.check_product_loyalty_export_limit', state.parameters))
        .then((response) => {
            if (!response.data.exceeds_limit) {
                state.isExportFileInProgress = false;
                return exportRecords(
...
const exportBoxProductCsvRecords = () => {
    state.isExportFileInProgress = true;
    return axios.get(route('warehouse_manager.products.check_box_product_export_limit', state.parameters))
        .then((response) => {
            if (!response.data.exceeds_limit) {
                state.isExportFileInProgress = false;
                return exportRecords(
...
const exportPDFRecords = (params, columns) => {
    ...
    printReport(route('warehouse_manager.products.print_products', params), props.exportPermission);
};
...
const showExportProductData = () => {
    router.get(route('warehouse_manager.export_records.index', { export_type: props.exportType }));
};
