<template>
    <PageTitle title="Edit Warehouse Manager" />
        ...
        <TabPanels class="mt-3 overflow-x-auto">
            <TabPanel class="active">
                <div class="intro-y box shadow-2xl bg-white rounded-lg">
                    ...
                    <form @submit.prevent="saveWarehouseManager()">
                        <div class="p-5">
                            ...
                            <div class="mt-5">
                                <Link :href="route('warehouse_manager.dashboard')">
                                    <SecondaryButton
                                        type="button"
                                        text="Cancel"
                                        class="w-24 mr-1"
                    ...
                    <div class="p-5">
                        <div class="grid grid-cols-12 gap-6">
                            <div class="col-span-12 sm:col-span-6">
                                ...
                                <JSwitch
                                    v-model:is-checked="warehouseManagerForm.enable2FA"
                                    input-label="Enable Google 2FA"
                                    validation-field-name="enable2FA"
                                    @update:is-checked="enableOrDisable2Fa(warehouseManagerForm.enable2FA)"
                                />
                            </div>
                            <div
                                v-if="warehouseManagerForm.enable2FA"
                            ...
                            >
                                <div class="flex flex-col items-center">
                                    ...
                                    <div v-if="state.setUpSuccess">
                                        <li
                                            v-for="(recoveryCode, index) in warehouseManagerForm.recovery_codes"
                                            :key="index"
                                        >
                                            {{ recoveryCode }}
                                        </li>
                                    ...
                                    <div
                                        v-if="state.setUpSuccess && warehouseManagerForm.recovery_codes"
                                        class="flex mt-6"
                                    >
...
const saveWarehouseManager = () => {
    warehouseManagerForm.put(route('warehouse_manager.update', props.warehouseManager.id));
    return;
};

const generateQrCode = () => {
    axios.post(route('warehouse_manager.generate2fa', props.warehouseManager.id), {
        ...warehouseManagerForm,
        two_factor_secret: warehouseManagerForm.two_factor_secret,
    }).then((response) => {
        state.qrCode = response.data.qrCodeSvg;
        warehouseManagerForm.two_factor_secret = response.data.secret;
        warehouseManagerForm.recovery_codes = response.data.recoveryCodes;
    }).catch((error) => {
        warehouseManagerForm.enable2FA = false;
...
const disable2FA = () => {
    ...
    axios.post(route('warehouse_manager.disable2fa', props.warehouseManager.id), warehouseManagerForm).then(() => {
        showSuccessNotification('2FA disabled successfully');
        state.qrCode = null;
        warehouseManagerForm.two_factor_secret = null;
        warehouseManagerForm.two_factor_qr_code = null;
        warehouseManagerForm.recovery_codes = null;
...
const verifyOtp = () => {
    axios.post(route('warehouse_manager.2fa.verify2fa', props.warehouseManager.id), { otp: state.otp, recovery_code: warehouseManagerForm.recovery_codes }).then((response) => {
...
onMounted(() => {
    if (props.warehouseManager) {
        Object.assign(warehouseManagerForm, props.warehouseManager);

        if (props.warehouseManager.two_factor_secret) {
            warehouseManagerForm.enable2FA = true;
            state.setUpSuccess = true;
        }
    }
});
