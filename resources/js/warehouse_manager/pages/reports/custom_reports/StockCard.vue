...
const exportPDFGeneralSalesReport = () => {
    if (validationCheck()) {
        showErrorNotification('Please select a Product and date before proceeding.');
        return;
    }
    ...
    if (!validFilters.includes(state.parameters.filter_by) &&
    ...
    printReport(route('warehouse_manager.custom_reports.print_stock_card', state.parameters));
};
...
const updateSelectedFilterId = (filterBy) => {
    ...
    if (filterBy === props.stockCardFilterStaticDetails.byBrand) {
        axios.post(route('warehouse_manager.brands.get_brands'))
            .then((response) => {
                state.brands = response.data.brands;
            });
    }
    if (filterBy === props.stockCardFilterStaticDetails.byDepartment) {
        axios.get(route('warehouse_manager.departments.get_departments_list'))
            .then((response) => {
                state.departments = response.data.departments;
            });
    }
    if (filterBy === props.stockCardFilterStaticDetails.byCategory) {
        axios.get(route('warehouse_manager.categories.get_categories_list')).then((response) => {
