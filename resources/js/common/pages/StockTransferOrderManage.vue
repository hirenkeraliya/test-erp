<template>
                    ...
                    <div class="p-5">
                        <div class="grid grid-cols-12 gap-0 sm:gap-6">
                            ...
                            >
                                ...
                                >
                                    Expected delivery date: {{ state.aggregateAverageDays }} day(s) - {{ getDateByAddDays(state.aggregateAverageDays) }} ({{ state.successRatio }}% Accurate)
                                </div>
                                <JTabs
                        ...
                        >
                            <div class="flex flex-wrap justify-between w-full mb-4 text-left sm:text-right">
                                <h4 class="self-center block text-base font-medium text-primary-p3">
                                    Items to transfer
                                </h4>
                                ...
                                <table class="table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="whitespace-nowrap">Package Type</th>
                                            <th class="whitespace-nowrap">Package Type Quantity</th>
                                            <th class="whitespace-nowrap">Total Quantity per Package Type</th>
                                            ...
                                            <td class="mt-10 whitespace-nowrap">
                                                <span v-if="stockTransferForm.transfer_items[itemIndex].product_id">
                                                    ...
                                                    <span class="font-medium">
                                                        ...
                                                        <Tippy
                                                            v-if="item.source_reserved_stock"
                                                            tag="label"
                                                    ...
                                                    <span class="font-medium">
                                                        ...
                                                        {{ stockTransferForm.transfer_items[itemIndex].product_uom }}
                                                        <Tippy
                                                            v-if="item.destination_reserved_stock"
                                                            tag="label"
                                            ...
                                            <td class="whitespace-nowrap">
                                                <span v-if="stockTransferForm.transfer_items[itemIndex].product_id">
                                                    ...
                                                    <span v-if="item.has_batch">
                                                        ...
                                                        <strong>
                                                            Transfer Stock:
                                                            {{ stockTransferForm.transfer_items[itemIndex].transfer_stock }}
                                                        </strong>
                                                    </span>

                                                    <input
                                                        v-if="parseFloat(item.source_stock) + parseFloat(item.initial_transfer_quantity) > 0 && !item.has_batch"
                                                        type="text"
    ...
    <JProductFilterDetails
        ...
        :product-search-url="productSearchUrlList"
    ...
    <BatchDetailsModal
        ...
        :batch-details="stockTransferForm.transfer_items[state.batchDetailsModalIndex].batch_details"
...
import { numberFormat, currentDate, getDateByAddDays } from '@commonServices/helper';
...
const props = defineProps({
    ...
    aggregateAverageDaysUrl: {
        type: String,
        required: true,
    },
    getInventoryStocksUrl: {
        type: String,
        required: true,
    },
    stockTransferStoreUrl: {
        type: String,
        required: true,
    },
    stockTransferUpdateUrl: {
        type: String,
        required: true,
    },
...
const state = reactive({
    ...
    displayAdvanceMatrixProductSearchModal: false,
...
const stockTransferForm = useForm({
    source_type_id: props.defaultLocationType,
    source_location_id: props.selectedLocationId,
    destination_type_id: props.defaultLocationType,
    ...
    transfer_type: props.stockTransferTypes.transfer_order,
    displayProductBatchDetailsModal: false,
    batchDetailsModalIndex: 0,
    aggregate_average_days:0,
});
...
const updatePackageTypeId = (packageTypeId, index) => {
    stockTransferForm.transfer_items[index].package_type_id = packageTypeId;
};

const updatePackageQuantity = (element, index) => {
    const inputValue = element.target.value ? element.target.value : 0;

    stockTransferForm.transfer_items[index].package_quantity = parseInt(inputValue);
};

const updatePackageTotalQuantity = (element, index) => {
    const inputValue = element.target.value;
    stockTransferForm.transfer_items[index].package_total_quantity = parseFloat(inputValue);
};
...
const getAggregateAverageDays = () => {
    if (stockTransferForm.source_location_id && stockTransferForm.destination_location_id) {
        ...
        axios.get(route(props.aggregateAverageDaysUrl), { params })
            .then((response) => {
                state.aggregateAverageDays = response.data.aggregate_average_days;
                state.successRatio = response.data.success_ratio;
                stockTransferForm.aggregate_average_days = state.aggregateAverageDays;
            })
            .catch((error) => {
...
const productSelected = (selectedProduct, index) => {
    ...
    if (selectedProduct) {
        ...
        stockTransferForm.transfer_items[index].remarks = null;
        stockTransferForm.transfer_items[index].has_batch = selectedProduct.has_batch;
...
const getSelectedProductsStock = () => {
    ...
    axios.get(route(props.getInventoryStocksUrl), { params })
        .then((response) => {
...
const saveStockTransfer = () => {
    if (props.stockTransfer) {
        ...
        stockTransferForm.put(route(props.stockTransferUpdateUrl, props.stockTransfer.data.id));
        return;
    }
    stockTransferForm.post(route(props.stockTransferStoreUrl));
};
...
const openProductBatchDetailsModal = (itemIndex) => {
    ...
    if (!stockTransferForm.transfer_items[state.batchDetailsModalIndex].batch_details.length) {
        stockTransferForm.transfer_items[state.batchDetailsModalIndex].batch_details = [
            {
                batch_number: null,
                quantity: null,
            }
...
const updateBatchDetails = (batchDetails) => {
    stockTransferForm.transfer_items[state.batchDetailsModalIndex].batch_details = batchDetails;
    stockTransferForm.transfer_items[state.batchDetailsModalIndex].transfer_stock = 0;

    stockTransferForm.transfer_items[state.batchDetailsModalIndex].batch_details.map((batch) => {
        stockTransferForm.transfer_items[state.batchDetailsModalIndex].transfer_stock += parseFloat(batch.quantity);
        return true;
    });

    stockTransferForm.transfer_items[state.batchDetailsModalIndex].package_total_quantity = stockTransferForm.transfer_items[state.batchDetailsModalIndex].transfer_stock;
};
...
const advanceFilterProductsSelection = (selectedProducts) => {
    ...
    for (const productKey in selectedProducts) {
        ...
        stockTransferForm.transfer_items.push({
            ...
            initial_transfer_quantity: 0,
            package_quantity: 0,
            package_total_quantity: selectedProducts[productKey].stock,
            remarks: null,
            unit_of_measure_derivative_id: null,
        });
        ...
        if (state.selectedProducts) {
            ...
            for (const key in state.selectedProducts) {
                stockTransferForm.transfer_items.push({
                    ...
                    initial_transfer_quantity: 0,
                    package_total_quantity: state.selectedProducts[key].quantity,
...
const calculateNewSourceStock = (sourceStock, initialTransferQuantity, transferStock, derivative) => {
    ...
    return numberFormat(
        (parseFloat(sourceStock) + parseFloat(initialTransferQuantity)) -
        (parseFloat(transferStock))
    );
};
...
const getHeadingText = () => {
    return props.stockTransfer ? 'Edit Transfer Order' : 'Add Transfer Order';
};
