        ...
        axios.post(route(props.productUpcUrl), {
            ...
            for (const key in products) {
                ...
                if (pageProps.value.product_variant) {
                    ...
                    unitOfMeasure = products[key].master_product.unit_of_measure ? products[key].master_product.unit_of_measure.name : null;
                    derivatives = products[key].master_product.unit_of_measure ? products[key].master_product.unit_of_measure.derivatives : null;

                } else {
                    productColor = products[key].color ? products[key].color.name : 'N/A';
                    productSize = products[key].size ? products[key].size.name : 'N/A';
                    unitOfMeasure = products[key].unit_of_measure ? products[key].unit_of_measure.name : null;
                    derivatives = products[key].unit_of_measure ? products[key].unit_of_measure.derivatives : null;
                }                
                
                matchedProductsList.push({
                    id: products[key].id, name: products[key].name, upc: products[key].upc, color_name: productColor, size_name: productSize, product_variant_values: productVariantValues, has_batch: products[key].has_batch, uom_name: unitOfMeasure, derivatives: derivatives
                });
            ...
            records.forEach((record) => response.data.products.forEach((product) => {
                if (record.upc.toString() === product.upc.toString()) {
                    ...
                    if (pageProps.value.product_variant) {
                        ...
                        unitOfMeasure = product.master_product.unit_of_measure ? product.master_product.unit_of_measure.name : null;
                        derivatives = product.master_product.unit_of_measure ? product.master_product.unit_of_measure.derivatives : null;
                        
                    } else {
                        productColor = product.color ? product.color.name : 'N/A';
                        productSize = product.size ? product.size.name : 'N/A';
                        unitOfMeasure = product.unit_of_measure ? product.unit_of_measure.name : null;
                        derivatives = product.unit_of_measure ? product.unit_of_measure.derivatives : null;
                    }

                    const recordDetails = { id: product.id, name: product.name, compound_product_name: product.compound_product_name, upc: product.upc, color_name: productColor, size_name: productSize, product_variant_values: productVariantValues, has_batch: product.has_batch, uom_name: unitOfMeasure, derivatives: derivatives};
