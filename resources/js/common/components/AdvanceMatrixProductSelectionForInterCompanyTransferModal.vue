...
const searchProductArticleNumber = () => {
    ...
    axios.post(route(props.productArticleSearchUrl), {
        ...
        location_id: props.purchaseOrderForm.location_id,
        external_location_id: props.purchaseOrderForm.external_location_id,
    }).then((response) => {
...
const updateTheStock = (product, transferStock) => {
    ...
    if (props.defaultOrderType !== props.staticDetails.transfer_request) {
        product.quantity = transferStock;
        return;
    }

    product.quantity = Math.min(transferStock, product.stock);
};
