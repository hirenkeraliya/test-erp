<template>
    <div class="py-5 md:py-0 overflow-x-hidden">
        <MobileMenu
            ...
            :home-url="route('super_admin.dashboard')"
            :menus="menus"
        ...
        <div class="content content--top-nav mx-5">
            <slot />
        </div>
...
<script setup>
import TopBar from '@superAdminComponents/TopBar.vue';
...
const getActiveMenuClass = (menu) => {
    ...
    if (menu.route_name === pageProps.value.current_route_name) {
...
const redirectToRoute = (menu) => {
    ...
    return route('super_admin.menu_page', { pageUrl: menu.title });
};

const handleScroll = () => {
    const scrollTimeoutDuration = 100;
