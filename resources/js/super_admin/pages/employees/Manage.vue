                ...
                >
                    <div class="p-5">
                        <div class="grid grid-cols-12 gap-0 sm:gap-6">
                            ...
                            <div class="input-form col-span-12 sm:col-span-6 md:col-span-6 lg:col-span-4 xl:col-span-3">
                                <FormSelectBox
                                    :selected-record="employeeForm.company_id"
                                    :records="companies"
                                    input-label="Company"
                                    validation-field-name="company_id"
                                    :required="true"
                                    @update:selected-record="setCompanyIdAndFetchDesignations($event)"
                                />
                            </div>
                    ...
                    <div class="p-5">
                        ...
                        <div class="mt-5">
                            <Link :href="route('super_admin.employees.index')">
                                <SecondaryButton
                                    type="button"
                                    text="Cancel"
                                    class="w-24 mr-1"
...
const saveEmployee = () => {
    if (props.employee) {
        employeeForm.post(route('super_admin.employees.update', props.employee.id));
        return;
    }

    router.post(route('super_admin.employees.store'), employeeForm);
};

const setCompanyIdAndFetchDesignations = (companyId) => {
    employeeForm.company_id = companyId;

    if (employeeForm.company_id) {
        axios.get(route('super_admin.designations.get_by_company', employeeForm.company_id)).then((response) => {
            if (response.data.data.length) {
                state.designations = response.data.data;
            }
        });

        axios.get(route('super_admin.employee_groups.get_employee_group_by_company', employeeForm.company_id)).then((response) => {
            if (response.data.data.length) {
                state.employeeGroups = response.data.data;
            }
        });
    }
};
...
onMounted(() => {
    if (props.employee) {
        setCompanyIdAndFetchDesignations(props.employee.company_id);
