<template>
    ...
    >
        ...
        >
            <div
                v-if="orderReturn.order_return_items && orderReturn.order_return_items.length"
            ...
            >
                ...
                <JSimpleTable
                    :columns="columnsForOrderReturnItemDetails"
                    :records="orderReturn.order_return_items"
                    :allow-search="true"
                >
                    <template #upc="data">
                    ...
                    <tbody>
                        <tr>
                            <td
                                ...
                                class="text-right"
                            >
                                {{ 'Total: ' + displayAmountWithCurrencySymbol(getTotalOf(orderReturn.order_return_items, 'subtotal')) }}
                            </td>
                        </tr>
                        <tr v-if="orderReturn.total_discount_amount > 0.0">
                            <td
                                colspan="8"
                                class="text-right"
                            >
                                {{ 'Discount: ' + displayAmountWithCurrencySymbol(orderReturn.total_discount_amount, true) }}
                            </td>
                        </tr>
                        <tr v-if="orderReturn.total_tax_amount > 0.0">
                            <td
                                colspan="8"
                                class="text-right"
                            >
                                {{ 'Tax: ' + displayAmountWithCurrencySymbol(orderReturn.total_tax_amount) }}
                            </td>
                        </tr>
                        <tr v-if="orderReturn.round_off > 0.0">
                            <td
                                colspan="8"
                                class="text-right"
                            >
                                {{ 'Round Off: ' + displayAmountWithCurrencySymbol(orderReturn.round_off) }}
                            </td>
                        </tr>
                        <tr>
                            <td
                                ...
                                class="text-right"
                            >
                                {{ 'Price Paid: ' + displayAmountWithCurrencySymbol(orderReturn.return_amount) }}
                            </td>
                        </tr>
                    </tbody>
                </table>
