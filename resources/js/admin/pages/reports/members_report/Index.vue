<template>
    ...
    <JTable
        ...
        :fetch-url="route('admin.members_report.fetch')"
        ...
        local-storage-key="admin-members-reports-columns"
        search-title="Search by member"
    >
...
const exportPDFRecords = (params, columns) => {
    ...
    printReport(route('admin.members_report.print_members_report', params), props.exportPermission);
};

const showMemberDetailsModal = (member) => {
    state.displayMemberDetailsModal = true;
    state.parameters.select_location_id = member.created_location_id;
    state.parameters.select_date = member.date;
    axios.get(route('admin.members_report.fetch_member_details', state.parameters)).then((response) => {
