<template>
    <TabGroup>
        <TabListForSellThroughReport report-title="Article Number" />
            ...
            <TabPanel>
                <div class="mt-10 grid grid-cols-1 gap-4 place-items-center">
                    <VerticalBarChart
                        ...
                        chart-id="by-article-number-bar-chart"
                        chart-title="Sell Through By Article Number"
                        class="w-full"
                        :data="isNotEmpty(state.chartRecords.sell_through) ? state.chartRecords.sell_through : [0]"
                        data-set-label="Sell Through By Article Number"
...
const exportCsvRecord = () => {
    return exportRecords(
        ...
        'sell-through-product-article-number.csv',
        state.parameters,
...
const exportExcelRecord = () => {
    return exportRecords(
        'export-sell-through-aggregate-report/',
        'sell-through-product-article-number.xlsx',
        state.parameters,
        props.exportPermission,
        state.parameters.export_columns
    );
...
const showBalanceDetailsModal = (articleNumber) => {
    state.parameters.articleNumber = articleNumber;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_balance_details_by_article_number';
    state.exportUrl = 'export-balance-details-by-article-number/';
    state.exportFileName = 'sell-through-balance-article-number';
    state.title = "Balance Details";
    state.modelColumns = state.balanceDetailsColumns;
    state.modelRecord = [];
    state.badgesTotals = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_balance_details_by_article_number', articleNumber), {
...
const showSoldDetailsModal = (articleNumber) => {
    state.parameters.articleNumber = articleNumber;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_sold_details_by_article_number';
    state.exportUrl = 'export-sold-details-by-article-number/';
    state.exportFileName = 'sell-through-sold-article-number';
    state.title = "Sold Details";
    state.modelColumns = state.soldDetailsColumns;
    state.modelRecord = [];
    state.badgesTotals = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_sold_details_by_article_number', articleNumber), {
        params: props.parameters,
    })
        .then((response) => {
            state.modelRecord = response.data.data;
...
const showReceivedDetailsModal = (articleNumber) => {
    state.parameters.articleNumber = articleNumber;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_received_details_by_article_number';
    state.exportUrl = 'export-received-details-by-article-number/';
    state.exportFileName = 'sell-through-received-article-number';
    state.title = "Received Details";
    state.modelColumns = state.receivedDetailsColumns;
    state.modelRecord = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_received_details_by_article_number', articleNumber), {
        params: props.parameters,
    })
        .then((response) => {
            state.modelRecord = response.data.data;
