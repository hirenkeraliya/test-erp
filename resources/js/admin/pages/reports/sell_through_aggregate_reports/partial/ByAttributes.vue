<template>
    <TabGroup>
        <TabListForSellThroughReport report-title="Attribute" />
                ...
                >
                    ...
                    <template #balance="record">
                        <div class="flex items-center gap-2">
                            ...
                            <Info
                                class="w-4 h-4"
                                @click="showBalanceDetailsModal(record.item.name)"
                            />
                        </div>
                    </template>
                    ...
                    <template #sold="record">
                        <div class="flex items-center gap-2">
                            ...
                            <Info
                                class="w-4 h-4"
                                @click="showSoldDetailsModal(record.item.name)"
                            />
                        </div>
                    </template>
                    ...
                    <template #received="record">
                        <div class="flex items-center gap-2">
                            ...
                            <Info
                                class="w-4 h-4"
                                @click="showReceivedDetailsModal(record.item.name)"
                            />
                        </div>
                    </template>
                </JTable>
            ...
            <TabPanel>
                <div class="mt-10 grid grid-cols-1 gap-4 place-items-center">
                    <VerticalBarChart
                        ...
                        chart-id="by-attributes-bar-chart"
                        chart-title="Sell Through By Attribute"
                        class="w-full"
                        :data="state.chartRecords.sell_through"
                        data-set-label="Sell Through By Attribute"
...
const exportCsvRecord = () => {
    return exportRecords(
        ...
        'sell-through-report-by-attribute.csv',
        state.parameters,
...
const exportExcelRecord = () => {
    return exportRecords(
        'export-sell-through-aggregate-report/',
        'sell-through-report-by-attribute.xlsx',
        state.parameters,
        props.exportPermission,
        state.parameters.export_columns
    );
...
const showBalanceDetailsModal = (attribute) => {
    state.parameters.attribute = attribute;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_balance_details_by_attribute';
    state.exportUrl = 'export-balance-details-by-attribute/';
    state.exportFileName = 'sell-through-balance-attribute';
    state.title = "Balance Details";
    state.modelColumns = state.balanceDetailsColumns;
    state.modelRecord = [];
    state.badgesTotals = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_balance_details_by_attribute', attribute), {
...
const showSoldDetailsModal = (attribute) => {
    state.parameters.attribute = attribute;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_sold_details_by_attribute';
    state.exportUrl = 'export-sold-details-by-attribute/';
    state.exportFileName = 'sell-through-sold-attribute';
    state.title = "Sold Details";
    state.modelColumns = state.soldDetailsColumns;
    state.modelRecord = [];
    state.badgesTotals = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_sold_details_by_attribute', attribute), {
        params: props.parameters,
    })
        .then((response) => {
            state.modelRecord = response.data.data;
...
const showReceivedDetailsModal = (attribute) => {
    state.parameters.attribute = attribute;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_received_details_by_attribute';
    state.exportUrl = 'export-received-details-by-attribute/';
    state.exportFileName = 'sell-through-received-attribute';
    state.title = "Received Details";
    state.modelColumns = state.receivedDetailsColumns;
    state.modelRecord = [];
    state.badgesTotals = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_received_details_by_attribute', attribute), {
        params: props.parameters,
    })
        .then((response) => {
            state.modelRecord = response.data.data;
