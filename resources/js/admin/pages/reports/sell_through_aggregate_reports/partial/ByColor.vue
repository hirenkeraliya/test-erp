<template>
    <TabGroup>
        <TabListForSellThroughReport report-title="Color" />
            ...
            <TabPanel>
                <div class="mt-10 grid grid-cols-1 gap-4 place-items-center">
                    <VerticalBarChart
                        ...
                        chart-id="by-colors-bar-chart"
                        chart-title="Sell Through By Color"
                        class="w-full"
                        :data="isNotEmpty(state.chartRecordForBars.sell_through) ? state.chartRecordForBars.sell_through : [0]"
                        data-set-label="Sell Through By Color"
...
import { exportRecords, printReport, sellThroughReportFilterValidationCheck } from '@commonServices/helper';
...
const exportSellThrough = () => {
    printReport(route('admin.sell_through_aggregate_reports.print_details', state.parameters), props.exportPermission);
};

const exportCsvRecord = () => {
    return exportRecords(
        'export-sell-through-aggregate-report/',
        'sell-through-report-by-color.csv',
        state.parameters,
...
const exportExcelRecord = () => {
    return exportRecords(
        'export-sell-through-aggregate-report/',
        'sell-through-report-by-color.xlsx',
        state.parameters,
        props.exportPermission,
        state.parameters.export_columns
    );
...
const showBalanceDetailsModal = (colorId) => {
    state.parameters.colorId = colorId;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_balance_details_by_color';
    state.exportUrl = 'export-balance-details-by-color/';
    state.exportFileName = 'sell-through-balance-color';
    state.title = "Balance Details";
    state.modelColumns = state.balanceDetailsColumns;
    state.modelRecord = [];
    state.badgesTotals = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_balance_details_by_color', colorId), {
...
const showSoldDetailsModal = (colorId) => {
    state.parameters.colorId = colorId;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_sold_details_by_color';
    state.exportUrl = 'export-sold-details-by-color/';
    state.exportFileName = 'sell-through-sold-color';
    state.title = "Sold Details";
    state.modelColumns = state.soldDetailsColumns;
    state.modelRecord = [];
    state.badgesTotals = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_sold_details_by_color', colorId), {
        params: props.parameters,
    })
        .then((response) => {
            state.modelRecord = response.data.data;
...
const showReceivedDetailsModal = (colorId) => {
    state.parameters.colorId = colorId;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_received_details_by_color';
    state.exportUrl = 'export-received-details-by-color/';
    state.exportFileName = 'sell-through-received-color';
    state.title = "Received Details";
    state.modelColumns = state.receivedDetailsColumns;
    state.modelRecord = [];
    state.badgesTotals = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_received_details_by_color', colorId), {
        params: props.parameters,
    })
        .then((response) => {
            state.modelRecord = response.data.data;
