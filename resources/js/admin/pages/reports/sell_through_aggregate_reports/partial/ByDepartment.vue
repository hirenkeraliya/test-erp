<template>
    <TabGroup>
        <TabListForSellThroughReport report-title="Departments" />
            ...
            <TabPanel>
                <div class="mt-10 grid grid-cols-1 gap-4 place-items-center">
                    <VerticalBarChart
                        ...
                        chart-id="by-department-bar-chart"
                        chart-title="Sell Through By Department"
                        class="w-full"
                        :data="isNotEmpty(state.chartRecords.sell_through) ? state.chartRecords.sell_through : [0]"
                        data-set-label="Sell Through By Department"
...
const exportCsvRecord = () => {
    return exportRecords(
        ...
        'sell-through-report-by-department.csv',
        state.parameters,
...
const exportExcelRecord = () => {
    return exportRecords(
        'export-sell-through-aggregate-report/',
        'sell-through-report-by-department.xlsx',
        state.parameters,
        props.exportPermission,
        state.parameters.export_columns
    );
...
const showBalanceDetailsModal = (departmentId) => {
    state.parameters.departmentId = departmentId;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_balance_details_by_department';
    state.exportUrl = 'export-balance-details-by-department/';
    state.exportFileName = 'sell-through-balance-department';
    state.title = "Balance Details";
    state.modelColumns = state.balanceDetailsColumns;
    state.modelRecord = [];
    state.badgesTotals = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_balance_details_by_department', departmentId), {
...
const showSoldDetailsModal = (departmentId) => {
    state.parameters.departmentId = departmentId;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_sold_details_by_department';
    state.exportUrl = 'export-sold-details-by-department/';
    state.exportFileName = 'sell-through-sold-department';
    state.title = "Sold Details";
    state.modelColumns = state.soldDetailsColumns;
    state.modelRecord = [];
    state.badgesTotals = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_sold_details_by_department', departmentId), {
        params: props.parameters,
    })
        .then((response) => {
            state.modelRecord = response.data.data;
...
const showReceivedDetailsModal = (departmentId) => {
    state.parameters.departmentId = departmentId;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_received_details_by_department';
    state.exportUrl = 'export-received-details-by-department/';
    state.exportFileName = 'sell-through-received-department';
    state.title = "Received Details";
    state.modelColumns = state.receivedDetailsColumns;
    state.modelRecord = [];
    state.badgesTotals = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_received_details_by_department', departmentId), {
        params: props.parameters,
    })
        .then((response) => {
            state.modelRecord = response.data.data;
