<template>
    <TabGroup>
        <TabListForSellThroughReport report-title="Location" />
            ...
            <TabPanel>
                <div class="mt-10 grid grid-cols-1 gap-4 place-items-center">
                    <VerticalBarChart
                        ...
                        chart-id="by-location-bar-chart"
                        chart-title="Sell Through By Location"
                        class="w-full"
                        :data="isNotEmpty(state.chartRecords.sell_through) ? state.chartRecords.sell_through : [0]"
                        data-set-label="Sell Through By Location"
...
const showBalanceDetailsModal = (locationId) => {
    state.parameters.locationId = locationId;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_balance_details_by_location';
    state.exportUrl = 'export-balance-details-by-location/';
    state.exportFileName = 'sell-through-balance-location';
    state.title = "Balance Details";
    state.modelColumns = state.balanceDetailsColumns;
    state.modelRecord = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_balance_details_by_location', locationId), {
...
const showSoldDetailsModal = (locationId) => {
    state.parameters.locationId = locationId;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_sold_details_by_location';
    state.exportUrl = 'export-sold-details-by-location/';
    state.exportFileName = 'sell-through-sold-location';
    state.title = "Sold Details";
    state.modelColumns = state.soldDetailsColumns;
    state.modelRecord = [];
    state.badgesTotals = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_sold_details_by_location', locationId), {
        params: props.parameters,
    })
        .then((response) => {
            state.modelRecord = response.data.data;
...
const showReceivedDetailsModal = (locationId) => {
    state.parameters.locationId = locationId;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_received_details_by_location';
    state.exportUrl = 'export-received-details-by-location/';
    state.exportFileName = 'sell-through-received-location';
    state.title = "Received Details";
    state.modelColumns = state.receivedDetailsColumns;
    state.modelRecord = [];
    state.badgesTotals = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_received_details_by_location', locationId), {
        params: props.parameters,
    })
        .then((response) => {
            state.modelRecord = response.data.data;
