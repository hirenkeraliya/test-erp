<template>
    <TabGroup>
        <TabListForSellThroughReport report-title="Style" />
            ...
            <TabPanel>
                <div class="mt-10 grid grid-cols-1 gap-4 place-items-center">
                    <VerticalBarChart
                        ...
                        chart-id="by-style-bar-chart"
                        chart-title="Sell Through By Style"
                        class="w-full"
                        :data="isNotEmpty(state.chartRecords.sell_through) ? state.chartRecords.sell_through : [0]"
                        data-set-label="Sell Through By Style"
...
const exportCsvRecord = () => {
    return exportRecords(
        ...
        'sell-through-report-by-style.csv',
        state.parameters,
...
const exportExcelRecord = () => {
    return exportRecords(
        'export-sell-through-aggregate-report/',
        'sell-through-report-by-style.xlsx',
        state.parameters,
        props.exportPermission,
        state.parameters.export_columns
    );
...
const showBalanceDetailsModal = (styleId) => {
    state.parameters.styleId = styleId;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_balance_details_by_style';
    state.exportUrl = 'export-balance-details-by-style/';
    state.exportFileName = 'sell-through-balance-style';
    state.title = "Balance Details";
    state.modelColumns = state.balanceDetailsColumns;
    state.modelRecord = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_balance_details_by_style', styleId), {
...
const showSoldDetailsModal = (styleId) => {
    state.parameters.styleId = styleId;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_sold_details_by_style';
    state.exportUrl = 'export-sold-details-by-style/';
    state.exportFileName = 'sell-through-sold-style';
    state.title = "Sold Details";
    state.modelColumns = state.soldDetailsColumns;
    state.modelRecord = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_sold_details_by_style', styleId), {
        params: props.parameters,
    })
        .then((response) => {
            state.modelRecord = response.data.data;
...
const showReceivedDetailsModal = (styleId) => {
    state.parameters.styleId = styleId;
    state.printUrl = 'admin.sell_through_aggregate_reports.print_received_details_by_style';
    state.exportUrl = 'export-received-details-by-style/';
    state.exportFileName = 'sell-through-received-style';
    state.title = "Received Details";
    state.modelColumns = state.receivedDetailsColumns;
    state.modelRecord = [];
    state.is_fetching = true,
    axios.get(route('admin.sell_through_aggregate_reports.fetch_received_details_by_style', styleId), {
        params: props.parameters,
    })
        .then((response) => {
            state.modelRecord = response.data.data;
