<template>
    ...
    >
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-5">
            ...
            <div>
                <JMultiSelect
                    :selected-records="state.parameters.promoters"
                    :records="state.promoters === null ? [] : state.promoters"
                    :placeholder="state.parameters.location_ids.length ? 'Please select Promoter' : 'Please select a Location First'"
                    :disabled="state.parameters.location_ids.length === 0"
    ...
    <JTable
        ...
        :fetch-url="route('admin.promoter_commission.fetch')"
        ...
        local-storage-key="admin-promoter-commission-reports-columns"
...
import PromoterCommissionDetails from '@adminPages/reports/promoter_commission/PromoterCommissionDetails.vue';
...
const clearAll = () => {
    ...
    state.parameters.promoters = null;
...
const updatePromoterId = (promoters) => {
    state.parameters.promoters = promoters;
...
const updateLocations = (locations) => {
    ...
    axios.post(route('admin.promoter_commission.get_promoters'), { location_ids: locationIds })
        .then((response) => {
...
const exportPdfPromoterCommission = (parameters, columns) => {
    ...
    printReport(route('admin.promoter_commission.print_promoter_commission', parameters), props.exportPermission);
};
