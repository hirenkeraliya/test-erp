    ...
    <div class="grid grid-cols-12 gap-0 sm:gap-6">
        ...
        >
            <FileUploadAndDisplayRecords
                ...
                product-upc-url="admin.products.get_matching_upc_products"
...
const validationCheck = () => {
    if (state.parameters.location_ids.length === 0) {
        return true;
    }
    ...
    if (state.parameters.filter_by === props.stockMovementFilterStaticDetails.byMasterProduct) {
        if (state.parameters.article_number === null) {
            return true;
        }
    }
...
const exportStockMovement = () => {
    if (validationCheck()) {
        if (state.parameters.filter_by === props.stockMovementFilterStaticDetails.byMasterProduct) {
            showErrorNotification('Please select a store, article number, date & report type before proceeding.');
            return;
        }
        showErrorNotification('Please select a store ,a date & report type before proceeding.');
        return;
    }
    ...
    printReport(route('admin.custom_reports.stock_movement_report_print', state.parameters));
};

const exportCsvRecord = () => {
    if (validationCheck()) {
        if (state.parameters.filter_by === props.stockMovementFilterStaticDetails.byMasterProduct) {
            showErrorNotification('Please select a store, article number and a date before proceeding.');
            return;
        }
