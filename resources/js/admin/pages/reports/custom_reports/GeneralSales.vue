...
const validationCheck = () => {
    if (state.parameters.location_ids === null) {
        return 'Please choose a locations before continuing';
    }

    if (props.generalSalesReportStaticDetails.byCurrentDayVsPreviousDay !== state.parameters.report_type && state.parameters.date_range === null) {
        return 'Please select a date range before continuing';
    }
    ...
    if (state.parameters.filter_by === props.generalSalesFilterStaticDetails.byPromoter && Object.keys(state.parameters.promoter_ids).length === 0) {
        return 'Please choose a promoter before continuing.';
    }

    if (state.parameters.filter_by === props.generalSalesFilterStaticDetails.byBrand && Object.keys(state.parameters.brand_ids).length === 0) {
        return 'Please choose a brand before continuing.';
    }

    if (state.parameters.filter_by === props.generalSalesFilterStaticDetails.byDepartment && Object.keys(state.parameters.department_ids).length === 0) {
        return 'Please choose a department before continuing.';
    }

    if (state.parameters.filter_by === props.generalSalesFilterStaticDetails.byCounter && Object.keys(state.parameters.counter_ids).length === 0) {
...
const exportPDFGeneralSalesReport = () => {
    ...
    printReport(route('admin.custom_reports.print_general_sales_report', state.parameters));
};
