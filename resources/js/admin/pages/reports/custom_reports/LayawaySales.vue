...
const updateLocationId = (locationIds) => {
    ...
    if (state.locationIds.length) {
        ...
        axios.post(route('admin.counters.get_counters_of_locations'), { location_ids: state.parameters.location_ids })
            .then((response) => {
...
const exportLayawaySales = () => {
    ...
    printReport(route('admin.custom_reports.layaway_sales_print', state.parameters));
};
