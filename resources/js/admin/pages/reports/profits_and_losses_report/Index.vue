<template>
    ...
    <JTable
        ...
        :fetch-url="route('admin.profits_and_losses_report.fetch')"
        ...
        local-storage-key="admin-profits-and-losses-report-columns"
    ...
    >
        <template #extra-header-data="record">
            <div class="block items-center xl:flex ml-0 sm:ml-3 mr-0 sm:mr-3 mb-2 sm:mb-0">
                ...
                <JBadge
                    ...
                    :label="' Units Returned: ' + truncateDecimal(record.data.total_units_return)"
                    class="mb-1 lg:mb-1 xl:mb-0"
                />

                <JBadge
                    v-if="record.data.total_sale_returns"
                    :label="'Returns: ' + displayAmountWithCurrencySymbol(record.data.total_sale_returns)"
                />

                <JBadge
                    v-if="record.data.total_purchase_cost"
                    :label="'Cost: ' + displayAmountWithCurrencySymbol(record.data.total_purchase_cost)"
                />

                <JBadge
                    v-if="record.data.total_profits_or_losses"
                    :label="'Profit/Loss: ' + displayAmountWithCurrencySymbol(record.data.total_profits_or_losses)"
                    class="mb-1 lg:mb-1 xl:mb-0"
                />
            </div>
        ...
        <template #total_purchase_cost="data">
            {{ displayAmountWithCurrencySymbol(data.item.total_purchase_cost) }}
        </template>

        <template #total_profit_or_loss="data">
            <p :class="data.item.total_profit_or_loss > 0 ? 'text-green-500' : 'text-red-500'">
                {{ displayAmountWithCurrencySymbol(data.item.total_profit_or_loss) }}
            </p>
        </template>

        <template #location="data">
            <div class="w-44">
                Sale Location: {{ data.item.location.sale_location }}
            </div>

            <div v-if=" data.item.location.sale_return_location !== null">
                Sale Return Location: {{ data.item.location.sale_return_location }}
            </div>
        </template>
    </JTable>
</template>
...
const exportCsvRecords = (params, columns) => {
    return exportRecords(
        'export-profits-and-losses-report/',
        'profits_and_losses_report.csv',
        params,
...
const exportExcelRecords = (params, columns) => {
    return exportRecords(
        'export-profits-and-losses-report/',
        'profits_and_losses_report.xlsx',
        params,
        props.exportPermission,
        columns
    );
...
const exportPDFRecords = (params, columns) => {
    ...
    printReport(route('admin.profits_and_losses_report.print', params), props.exportPermission);
};
...
const refreshPage = () => {
    router.get(route('admin.profits_and_losses_report.index'));
};
