<template>
    ...
    <div
        v-if="state.displayStockTransfersFilter"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-5">
            ...
            <div>
                <FormSelectBox
                    ...
                    :records="state.parameters.location_id ? allTransferTypes : transferTypes"
            ...
            <div>
                <JSwitch
                    ...
                    input-label="Smart Transfer"
                    :is-checked="state.parameters.select_status === statuses.system_generated"
                    class="mt-[0px]"
                    @update:is-checked="updateSmartTransfer($event)"
                />
            </div>
        </div>
    ...
    <JTable
        :fetch-url="route('admin.stock_transfers.fetch', { stock_transfer_number: stockTransferNumber, stock_transfer_id: stockTransferId, dashboard_transfer_type: dashboardFilterData.transfer_type })"
        ...
        <template #actions="data">
            <div class="flex justify-center items-center">
                ...
                >
                    ...
                    <DropdownMenu
                        v-if="data.item.status_id === statuses.draft || data.item.status_id === statuses.system_generated"
                        class="w-60"
                    >
                        <DropdownContent>
                            <DropdownItem @click="edit(data.item.id, dismiss)">
                                <CheckSquare class="w-4 h-4 mr-2" />Edit
                            </DropdownItem>

                            <DropdownItem
                                v-if="data.item.transfer_type === stockTransferTypes.transfer_order"
                                @click="printStockTransfer(data.item.id, 'OUT', dismiss)"
                            >
                            ...
                            <DropdownItem
                                v-if="data.item.transfer_type === stockTransferTypes.request_order"
                                @click="printStockTransfer(data.item.id, 'OUT', dismiss)"
                            >
                                <Printer class="w-4 h-4 mr-1" />
                                Print Request Order
                            ...
                            <DropdownItem
                                v-if="data.item.status_id === statuses.draft"
                            ...
                            <DropdownItem
                                v-if="data.item.status_id === statuses.system_generated"
                                @click="updateStatus(data.item.id, statuses.open, dismiss)"
                            >
                    ...
                    <DropdownMenu class="w-60">
                        <DropdownContent
                            v-if="data.item.transfer_type === stockTransferTypes.request_order &&
                                data.item.status_id === statuses.approved"
                        >
                            <DropdownItem @click="printStockTransfer(data.item.id, 'IN', dismiss)">
                                <Printer class="w-4 h-4 mr-1" />
                                Print Transfer In
                            </DropdownItem>

                            <DropdownItem @click="printStockTransfer(data.item.id, 'OUT', dismiss)">
                                <Printer class="w-4 h-4 mr-1" />
                                Print Transfer Out
                            </DropdownItem>

                            <DropdownItem @click="markAsShipped(data.item.id, dismiss, data.item.source_id, data.item.destination_id)">
                        ...
                        <DropdownContent
                            v-if="data.item.status_id === statuses.shipped"
                        >
                            <DropdownItem
                        ...
                        <DropdownContent
                            v-if="data.item.status_id === statuses.received"
                        >
                            <DropdownItem
                                @click="deliveryNote(data.item.id, dismiss)"
                            >
                                <CheckSquare class="w-4 h-4 mr-2" />Add Delivery Notes
                            </DropdownItem>
                        </DropdownContent>

                        <DropdownContent
                            v-if="data.item.status_id === statuses.transit"
                        >
                            <DropdownItem
                                @click="updateStatus(data.item.id, statuses.transit_in, dismiss)"
                            >
                                <CheckSquare class="w-4 h-4 mr-2" />Mark as Transit IN
                            </DropdownItem>
                            ...
                            <DropdownItem
                                @click="printStockTransfer(data.item.id, 'IN', dismiss)"
                            >
                                <Printer class="w-4 h-4 mr-1" />
                                Print Transfer In
                            </DropdownItem>
                        ...
                        <DropdownContent
                            v-if="data.item.status_id === statuses.transit_in"
                        >
                            <DropdownItem
                                @click="updateStatus(data.item.id, statuses.transit_out, dismiss)"
                            >
                                <CheckSquare class="w-4 h-4 mr-2" />Mark as Transit OUT
                            </DropdownItem>
                        ...
                        <DropdownContent
                            v-if="data.item.status_id === statuses.transit_out"
                        >
                            <DropdownItem
                                @click="updateStatus(data.item.id, statuses.received, dismiss)"
                            >
                        ...
                        <DropdownContent
                            v-if="data.item.transfer_type === stockTransferTypes.transfer_order &&
                                data.item.status_id === statuses.open"
                        >
                            <DropdownItem
                                @click="printStockTransfer(data.item.id, 'OUT', dismiss)"
                            >
                            ...
                            <DropdownItem
                                class="text-danger"
                                @click="cancelStatus(data.item.id, statuses.cancelled, dismiss)"
                            >
                                <X class="w-4 h-4 mr-2" /> Cancel
                            </DropdownItem>
                        </DropdownContent>
                        ...
                        <DropdownContent
                            v-if="data.item.status_id === statuses.discrepancy"
                        >
                            <DropdownItem
                                @click="printStockTransfer(data.item.id, 'OUT', dismiss)"
                            >
                        ...
                        <DropdownContent
                            v-if="(data.item.status_id === statuses.rejected ||
                                data.item.status_id === statuses.cancelled ||
                                data.item.status_id === statuses.closed
                            )"
                        >
                            <DropdownItem
                                @click="printStockTransfer(data.item.id, 'OUT', dismiss)"
        ...
        <template #extra-header-data="record">
            <div class="mx-0 mb-2 sm:mb-0 md:mx-2">
                <div class="flex justify-between items-center content-center">
                    <div>
                        <div
                            v-if="getBadgeDisplay(record.data.transferOrderStatusCounts, staticStockTransferType.transferType)"
                            class="block items-center xl:flex"
                        >
                            <label class="mr-2 font-semibold">Transfer Order : </label>

                            <div>
                                <div class="block items-center xl:flex mr-2 mb-2">
                                    <JBadge
                                        ...
                                        @click="statusChanges(statusCount.id, staticStockTransferType.transferType)"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div
                            ...
                            class="block items-center xl:flex"
                        >
                            <label class="mr-2 font-semibold">Request Order : </label>
            ...
            <p class="text-lg font-bold mr-2 mb-2 sm:mb-0 float-left sm:float-none">
                <OutlinePrimaryButton
                    text="Filters"
                    class="text-sm shadow-md"
                    @click="state.displayStockTransfersFilter = !state.displayStockTransfersFilter"
                />
            </p>
        </template>
    </JTable>
    ...
    <StockTransferReceivedDate
        ...
        route-url="admin.stock_transfers.update_received_date_and_status"
    ...
    <StockTransferShipped
        ...
        route-url="admin.stock_transfers.mark_as_shipped_or_transit"
    ...
    <StockTransferCancelRemarks
        ...
        route-url="admin.stock_transfers.update_status"
...
import { Check, CheckSquare, Download, GitMerge, Info, List, MoreHorizontal, PlayCircle, Printer, Truck, X } from 'lucide-vue-next';
...
const state = reactive({
    ...
    parameters: {
        transfer_type: !props.dashboardFilterData.is_from_stock_overview ? props.dashboardFilterData.transfer_type : null,
    ...
    displayStockTransfersFilter: false,
...
const openStockTransferItemsModal = (data) => {
    ...
    axios.get(route('admin.stock_transfers.fetch_stock_transfer_items', data.id))
        .then((response) => {
...
const revertBackToDirect = (stockTransferId, dismiss) => {
    ...
    confirmDialogBox(message, () => {
        ...
        axios.post(route('admin.stock_transfers.mark_as_shipped_or_transit', stockTransferId), data)
            .then((response) => {
...
const redirectToAddStockTransfer = (transferType) => {
    router.get(route('admin.stock_transfers.create', transferType));
};
...
const closeDiscrepancy = (stockTransferId, dismiss) => {
    router.get(route('admin.stock_transfers.discrepancy', stockTransferId));
    dismiss();
};

const printStockTransfer = (stockTransferId, transferType) => {
    printReport((route('admin.stock_transfers.print_stock_transfer', [stockTransferId, transferType])), props.exportPermission);
};
...
const edit = (stockTransferId, dismiss) => {
    router.get(route('admin.stock_transfers.edit', stockTransferId));
    dismiss();
};

const deliveryNote = (stockTransferId, dismiss) => {
    router.get(route('admin.stock_transfers.delivery_note', stockTransferId));
    dismiss();
};
...
const updateSmartTransfer = (smartTransfer) => {
    if (smartTransfer) {
        state.parameters.select_status = props.statuses.system_generated;
        refreshTable();
        return;
    }
...
const markAsApproved = (stockTransferId, dismiss) => {
    router.get(route('admin.stock_transfers.shipping_details', stockTransferId));
    dismiss();
};
...
const editRequestOrderByDestination = (stockTransferId, dismiss) => {
    router.get(route('admin.stock_transfers.edit_request_order', stockTransferId));
    dismiss();
};
...
const getStatusColor = (status) => {
    ...
    if (status === props.staticTitleStatuses.open || status === props.staticTitleStatuses.draft || status === props.staticTitleStatuses.system_generated || status === props.staticTitleStatuses.shipped) {
...
const getFilterTabStatus = () => {
    return state.displayStockTransfersFilter === false && (state.parameters.transfer_type !== null || state.parameters.select_status !== null);
};
...
const refreshPage = () => {
    router.get(route('admin.stock_transfers.index'));
};
...
onMounted(() => {
    if (props.stockTransferNumber) {
        state.isClear = true;
        state.displayStockTransfersFilter = true;
        refreshTable();
    }
