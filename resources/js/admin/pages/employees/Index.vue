<template>
    ...
    <JTable
        :fetch-url="route('admin.employees.fetch')"
    ...
    >
        ...
        <template #email="data">
            {{ data.item.email }}
            <Tippy
                v-if="data.item.email && ! data.item.is_email_verified"
        ...
        <template #action="data">
            <div class="flex justify-center items-center">
                <Link
                    ...
                    :href="route('admin.employees.edit', data.item.id)"
                >
                ...
                <Link
                    ...
                    class="flex items-center mr-3"
                    :href="route('admin.employees.resend_verification_email', data.item.id)"
                >
                    <Tippy
                        :content="'Resend mail'"
                    >
...
import { CheckSquare, Info, Mail, TriangleAlert } from 'lucide-vue-next';
...
const setStatus = (employeeId, status) => {
    router.post(route('admin.employees.set_status', [employeeId, status ? 1 : 0]));
};
