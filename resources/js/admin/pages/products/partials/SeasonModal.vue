<template>
    <Modal
        size="modal-lg"
        :show="seasonModalShow"
        @hidden="hideSeasonModal"
    >
        <ModalHeader>
...
const emits = defineEmits([
    'update:hide-season-modal',
    'new:record',
]);

const hideSeasonModal = () => {
    emits('update:hide-season-modal', false);
};
...
const saveSeason = () => {
    axios.post(route('admin.seasons.store_return'), seasonForm)
        .then((response) => {
            emits('new:record', response.data.season);
            hideSeasonModal();
        }).catch((error) => {
