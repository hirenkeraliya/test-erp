<template>
    <div class="top-bar-boxed h-[55px] sm:h-[55px] md:h-[65px] z-[51] border-b border-white/[0.08] mt-16 md:mt-0 -mx-3 sm:-mx-8 md:-mx-0 px-3 md:border-b-0 relative md:fixed md:inset-x-0 md:top-0 sm:px-8 md:px-10 md:pt-10 md:bg-gradient-to-b md:from-slate-100 md:to-transparent before:h-[65px] after:h-[65px]">
        <div class="h-auto sm:h-auto md:h-full flex items-center">
            ...
            <Notification
                mark-all-as-read-url="admin.notifications.mark_all_as_read"
                fetch-url="admin.notifications.fetch"
                fetch-read-notification-url="admin.notifications.fetch_read_notification"
                mark-as-read-url="admin.notifications.mark_as_read"
                mark-as-unread-url="admin.notifications.mark_as_unread"
            />
...
const state = reactive({
    ...
    localStorageName: 'admin_recently_visited_page',
});

const changePassword = (dismiss) => {
    router.get(route('admin.change_password'));
    dismiss();
};

const editProfile = (dismiss) => {
    router.get(route('admin.edit_profile'));
    dismiss();
};
...
setInterval(() => {
    const timeIncrementSeconds = 10;
    const currentTime = new Date(state.currentDateTime);
    currentTime.setSeconds(currentTime.getSeconds() + timeIncrementSeconds);
