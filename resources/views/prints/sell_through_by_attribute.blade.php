...
<body class="arial-font-custom-report">
    ...
    <h4>
        <strong>Sell Through Through Report {{ $reportType }}</strong>
    </h4>
    ...
    <table class="table table-bordered bordered">
        ...
        <tbody>
            @forelse($sellThroughDataByAttributes as $sellThroughDataByAttribute)
                <tr class="page-break-inside-avoid">
                    @foreach($columns as $column)
                        <td class="{{ $column['bodyClass'] }}">
                            {{ $sellThroughDataByAttribute[$column['key']] }}
                        </td>
                    @endforeach
                </tr>
            @empty
            ...
            <tr class="page-break-inside-avoid text-bold">
                ...
                @foreach (array_slice($columns, $colspan) as $column)
                    <td class="{{ $column['bodyClass'] }}">
                        <b>
                            {{ $sellThroughTotalDataByAttributes[$column['key']] }}
                        </b>
                    </td>
                @endforeach
            </tr>
