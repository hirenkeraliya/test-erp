...
<head>
    ...
    <title>Sell Through Aggregate</title>
</head>
...
<body class="arial-font-custom-report">
    ...
    <table class="table table-bordered bordered">
        ...
        <tbody>
            @forelse($sellThroughDataByColors as $sellThroughDataByColor)
                <tr class="page-break-inside-avoid">
                    @foreach ($columns as $column)
                        <td class="{{ $column['bodyClass'] }}">
                            {{ $sellThroughDataByColor[$column['key']] }}
                        </td>
                    @endforeach
                </tr>
            @empty
            ...
            <tr class="page-break-inside-avoid text-bold">
                ...
                @foreach (array_slice($columns, $colspan) as $column)
                    <td class="{{ $column['bodyClass'] }}">
                        {{ $sellThroughTotalDataByColors[$column['key']] }}
                    </td>
                @endforeach
            </tr>
        </tbody>
    ...
    <script type="text/javascript">
        ...
        var chartData = {!! isset($chartRecords['sell_through']) ? json_encode($chartRecords['sell_through']) : '[]' !!};
        var chartColor = {!! json_encode($chartRecordsPieColors) !!};
        ...
        var chartLabelForBar = {!! isset($chartRecordForBars['labels']) ? json_encode($chartRecordForBars['labels']) : '[]' !!};
        var chartDataForBar = {!! isset($chartRecordForBars['sell_through']) ? json_encode($chartRecordForBars['sell_through']) : '[]' !!};

        chartLabelForBar = chartLabelForBar.length > 0 ? chartLabelForBar : ['No Records'];
        chartDataForBar = chartDataForBar.length > 0 ? chartDataForBar : [10];

        const preparedRecordForBars = [];
        for (const key in chartLabelForBar) {
            preparedRecords.push({
                name: chartLabelForBar[key],
                value: chartDataForBar[key]
            });
        }
