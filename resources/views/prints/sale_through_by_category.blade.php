...
<body class="arial-font-custom-report">
    ...
    <table class="table table-bordered bordered">
        ...
        <tbody>
            @forelse($saleThroughDataByCategories as $saleThroughDataByCategory)
                <tr class="page-break-inside-avoid">
                    <td>
                        {{ $saleThroughDataByCategory['name'] }}
                    </td>

                    <td class="text-right">
                        @truncateDecimal($saleThroughDataByCategory['received'])
                    </td>

                    <td class="text-right">
                        @truncateDecimal($saleThroughDataByCategory['sold'])
                    </td>

                    <td class="text-right">
                        @truncateDecimal($saleThroughDataByCategory['returned'])
                    </td>

                    <td class="text-right">
                        @truncateDecimal((float) $saleThroughDataByCategory['received'] - ((float) $saleThroughDataByCategory['sold'] - (float) $saleThroughDataByCategory['returned']))
                    </td>

                    <td class="text-right">
                        @if ((float) $saleThroughDataByCategory['received'] === (float) 0)
                            0.00
                        @else
                            @truncateDecimal((((float) $saleThroughDataByCategory['sold'] - (float) $saleThroughDataByCategory['returned']) * 100) / (float) $saleThroughDataByCategory['received'])
                        @endif
                    </td>
                </tr>
            @empty
            ...
            <tr class="page-break-inside-avoid text-bold">
                ...
                <td class="text-right">
                    <b>
                        @truncateDecimal($saleThroughTotalDataByCategories['received'])
                    </b>
                </td>

                <td class="text-right">
                    <b>
                        @truncateDecimal($saleThroughTotalDataByCategories['sold'])
                    </b>
                </td>

                <td class="text-right">
                    <b>
                        @truncateDecimal($saleThroughTotalDataByCategories['returned'])
                    </b>
                </td>

                <td class="text-right">
                    <b>
                        @truncateDecimal($saleThroughTotalDataByCategories['remaining'])
                    </b>
                </td>

                <td class="text-right">
                    <b>
                        @truncateDecimal($saleThroughTotalDataByCategories['sale_through'])
                    </b>
                </td>
            </tr>
        </tbody>
        ...
        pieChartElement.setOption({
            tooltip: {
                valueFormatter: function(value) {
                    return formatLabelForChartWithPercentage(value);
                }
            },
            legend: {
                data: chartLabel,
